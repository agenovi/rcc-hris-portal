...
function renderDashboard(){
  ...
  // merchandiser pay
  const rates=DISERS.filter(d=>(d.status||'').toLowerCase().startsWith('active')&&d.total_rate).map(d=>Number(d.total_rate));
  ...
  pg.innerHTML=`
    ...
    <div class="grid kpis" style="margin-top:13px;">
      ...
      <div class="kpi warn" style="cursor:pointer;" onclick="go('manning')"><div class="k-l">Headcount Deficiency</div><div class="k-n">${merchDef}</div><div class="k-s">unfilled merchandiser slots</div></div>
    ...
    <div class="two-col">
      <div class="panel" style="margin-top:0;">
        ...
        <div class="task" style="cursor:pointer;" onclick="go('manning')"><div class="dot r"></div><div><div class="tt">${merchDef} merchandiser slots to fill</div><div class="td">${totAHC} approved vs ${totCHC} confirmed — across ${SCs.length} Sales Coordinators</div></div><div class="due r">staffing</div></div>
      </div>
      <div>
        <div class="panel" style="margin-top:0;">
        ...
        <div class="panel">
          ...
          <div class="team-row"><div class="tn">Merchandiser staffing</div><span class="mini bad">${merchDef} short</span></div>
        </div>
        <div class="panel">
    ...
    <div class="two-col">
      <div class="panel" style="margin-top:0;">
        ...
        <div class="psub">Open stores under each SC</div>
        ${chartRows(bySC)}
      </div>
      <div class="panel" style="margin-top:0;">
    ...
    <div class="two-col">
      ...
      <div class="panel" style="margin-top:0;">
        <h2>Merchandiser pay</h2>
        <div class="psub">Daily rate across ${rates.length} merchandisers on file</div>
        <div class="grid kpis" style="grid-template-columns:repeat(3,1fr);">
          <div class="kpi"><div class="k-l">Average</div><div class="k-n">₱${avgRate.toLocaleString()}</div></div>
          <div class="kpi"><div class="k-l">Lowest</div><div class="k-n">₱${minRate.toLocaleString()}</div></div>
          <div class="kpi"><div class="k-l">Highest</div><div class="k-n">₱${maxRate.toLocaleString()}</div></div>
        </div>
        <div class="task" style="margin-top:10px;"><div class="dot g"></div><div><div class="tt">${DISERS.length} active merchandisers tracked</div><div class="td">With type, position, rate, contract dates &amp; status — drill in from Manning or Worksites</div></div></div>
      </div>
    </div>`;
}
  ...
  const cols=PH_PHASES.map(ph=>{
    ...
    return `<div class="col"><div class="col-h">${ph.label}<span>${cards.length} · ${esc(ph.actor)}</span></div>
      ${cards.map(c=>`<div class="ccard clickable" data-id="${c.id}" ${ph.key==="READY_FOR_CONTRACT"?'style="border-color:#bcdcc7;background:var(--green-light);"':''}>
        ...
        <div class="cd">${esc(c.position||"—")} · ${esc(c.hire_source||"Direct")}${c.assessment_score!=null?` · exam ${c.assessment_score}`:""}${c.sm_acceptance&&c.sm_acceptance!=="NA"?` · SM ${esc(c.sm_acceptance)}`:""}</div></div>`).join("")
...
function openPrehire(c){
  ...
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:580px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    ...
    <div style="padding:18px 22px 60px;">
      <div class="panel" style="margin-top:0;">
        <div class="esec-h"><span class="esec-n">i</span><span class="esec-t">Application</span></div>
        ${phRow("Email",c.email)}${phRow("Phone",c.phone)}${phRow("Position",c.position)}${phRow("Department",c.department)}${phRow("Worksite",c.worksite)}${phRow("Hire source",c.hire_source||"Direct")}
      </div>
      <div class="panel">
        <div class="esec-h"><span class="esec-n">ii</span><span class="esec-t">Assessment &amp; Offer</span></div>
        ${phRow("Assessment",c.assessment_type)}${phRow("Score",c.assessment_score)}${phRow("Passed",c.assessment_passed==null?"":(c.assessment_passed?"Yes":"No"))}${phRow("Contract type",c.contract_type)}${phRow("Daily rate",c.daily_rate?("₱"+Number(c.daily_rate).toLocaleString()):"")}
      </div>
      <div class="panel">
        <div class="esec-h"><span class="esec-n">iii</span><span class="esec-t">Review, Gates &amp; Onboarding</span></div>
        ${phRow("HR officer notes",c.hr_officer_notes)}${phRow("Juvelyn sign-off",c.juvelyn_review_date?"Signed":"")}${phRow("SM acceptance (hard gate)",c.sm_acceptance)}${phRow("Contract ID",c.contract_id)}${phRow("PayPlus setup",c.payplus_setup?"Done":"")}${phRow("Assigned Employee ID",c.assigned_employee_id)}
      </div>
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:14px;">
        ${next?`<button class="btn" id="phAdvance">Advance → ${esc(next.label)}</button>`:'<span class="pill active">Pipeline complete</span>'}
