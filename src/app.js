...
const ROLE_BY_EMAIL={ "anj@hassarams.com":"admin", "sanjay@hassarams.com":"admin", "hr3@hassarams.com":"payroll", "hr4@hassarams.com":"manager", "hr@hassarams.com":"relations" };  // hr3@=Grazel(payroll) · hr4@=Rhel(HR Manager) · hr@=Juvy(HR Relations)
...
const REAL_PAGES=new Set(["dashboard","employees","branches","manning","prehire","onboarding","exit","contracts","loans","compliance","settings","signatures","memos","evaluations","reports","activity","maternity"]);
...
function claimCard(s){ const d=s.details||{}; const P=n=>"₱"+Number(n||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2});
  ...
  return `<div style="background:linear-gradient(135deg,#12352a,#1c4b39);margin:-22px -22px 0;padding:15px 20px;border-radius:14px 14px 0 0;color:#fff;">
    ...
    <div style="padding:16px 2px 0;">
      ...
      <details style="margin:11px 0 2px;"><summary style="cursor:pointer;font-size:12.5px;color:#1E3A5F;font-weight:600;">View full line-by-line breakdown</summary>
...
function renderManning(){
  ...
  const paint=()=>{
    ...
    $("#scBlocks").innerHTML=list.map(sc=>{
      ...
      const st=scStatus(sc), gone=scIsGone(sc), snote=(SC_STATUS[sc]&&SC_STATUS[sc].note)||"";
      return `<div class="panel" style="margin-top:14px;${gone?'border:1.5px solid #e5b4ae;background:#fdf4f2;':''}">
        <div class="sc-card">
          <div class="sc-av"${gone?' style="background:var(--red);"':''}>${esc(sc[0])}</div>
          <div style="flex:1;"><div class="sc-l">Sales Coordinator</div><div class="sc-n">${esc(sc)}${gone?` <span class="pill awol">${st==="Pending"?"Replacement pending":"Vacant — needs replacement"}</span>`:""}</div>${gone&&snote?`<div style="font-size:11.5px;color:var(--red);margin-top:2px;">${esc(snote)}</div>`:""}</div>
          ...
          ${canManageStores()?`<button class="btn ghost" data-scstatus="${esc(sc)}" style="flex-shrink:0;${gone?'color:var(--green-dark);':'color:var(--red);border-color:#f1c9c5;'}">${gone?"Mark filled":"Coordinator left"}</button>`:""}
        </div>
        ${gone?`<div class="psub" style="margin:2px 0 8px;color:#8a3a30;">⚠ This coordinator has left — the ${stores.length} store${stores.length>1?'s':''} below currently have no active SC. Reassign or backfill.</div>`:""}
        <table>
...
async function saveEmployee(id,modal){
  ...
  if(id){
    ...
    if(ch.length) await logChange("employee",id,p.full_name,"Sensitive field edited",ch.join(" · "));
  } else { await logChange("employee",null,p.full_name,"Added",""); }
...
function computeMaternity(c){
  ...
  const useActual=(c.full_pay_basis==="Actual")&&Number(c.actual_avg_days)>0;
  const effDays=useActual?Number(c.actual_avg_days):dpm;          // actual PayPlus attendance vs theoretical days/month
  const monthly=dr*effDays, fullPayDaily=monthly/30, fpmb=fullPayDaily*days;
  const msc=matMSC(c.msc).sort((a,b)=>b-a), sum6=msc.slice(0,6).reduce((s,x)=>s+x,0), adsc=sum6/180;
  ...
  const employerShare=Math.max(0,fpmb-sssBenefit), gross=sssBenefit+employerShare;
  const deductions=["deduct_sss","deduct_philhealth","deduct_pagibig","deduct_loans","deduct_advance","deduct_other"].reduce((s,k)=>s+(Number(c[k])||0),0);
  return { dpm,dr,days,effDays,useActual,monthly,fullPayDaily,fpmb,msc,sum6,adsc,sssBenefit,employerShare,gross,deductions,net:gross-deductions,override:ov!=null };
}
function renderMaternity(){
  ...
  pg.innerHTML=`
    <div class="panel" style="margin-top:0;">
      ...
      <div class="psub">RA 11210 · 105-Day Expanded Maternity Leave. RCC standard: <b>Full-pay maternity benefit</b> vs <b>SSS benefit</b> → employer pays the <b>differential only when full pay is higher</b> (for most daily-paid disers the SSS benefit is higher, so employer share = ₱0). Salary-restricted — visible to you &amp; Grazel only.</div>
...
function openMaternityForm(c){
  ...
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:600px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    ...
    <div id="matForm" style="padding:16px 20px 70px;">
      ...
      <div class="panel"><div class="subhead">C · Salary basis → full pay</div>
        <div class="form-grid">${mf("mat_dr","Daily salary rate (₱)",c.daily_rate)}${sel("mat_basis","Full-pay basis",["Contractual","Actual"],c.full_pay_basis||"Contractual")}</div>
        <div id="mat_basisC">
          <div id="mat_contractRow" style="display:${(c.full_pay_basis==="Actual")?'none':'block'};">${mf("mat_dpm","Days / month (theoretical)",c.days_per_month||26)}
            <div class="psub" style="margin:-4px 0 4px;">Contractual = rate × days/month ÷ 30. Assumes a full month with no absences/late/undertime.</div></div>
          <div id="mat_actualRow" style="display:${(c.full_pay_basis==="Actual")?'block':'none'};">
            <button class="btn ghost" id="mat_pullAtt" type="button">↻ Pull actual attendance from PayPlus</button>
            <div id="mat_attBox" style="margin-top:8px;">${c.actual_avg_days?matAttSummaryHtml(c.attendance,c.actual_avg_days):'<div class="psub">Pulls the 6 completed months before the delivery month — present days net of late/undertime — for her true average.</div>'}</div>
          </div>
        </div>
      </div>
      ...
      <div class="panel"><div class="subhead">E · Deductions during the leave months</div>
        ...
        <div class="form-grid">${mf("mat_deduct_pagibig","Pag-IBIG share",c.deduct_pagibig)}${mf("mat_deduct_loans","SSS / Pag-IBIG loan",c.deduct_loans)}</div>
        <div class="form-grid">${mf("mat_deduct_advance","Company cash advance",c.deduct_advance)}${mf("mat_deduct_other","Other",c.deduct_other)}</div>
      </div>
  ...
  const basisSel=document.getElementById("mat_basis");
  basisSel.addEventListener("change",()=>{ const act=basisSel.value==="Actual"; document.getElementById("mat_contractRow").style.display=act?"none":"block"; document.getElementById("mat_actualRow").style.display=act?"block":"none"; matRecalc(); });
  const pullBtn=document.getElementById("mat_pullAtt"); if(pullBtn) pullBtn.addEventListener("click",matPullAttendance);
  document.getElementById("matForm").addEventListener("input",matRecalc);
  matRecalc();
}
// Convert late+undertime minutes to day-equivalents (8h day) and net them off present days.
function matAvgFromAttendance(months){
  const m=(months||[]).filter(x=>x.hasData&&Number(x.present)>0);
  if(!m.length) return null;
  const perMonth=m.map(x=>{ const lost=(Number(x.lateMinutes||0)+Number(x.undertimeMinutes||0))/480; return Math.max(0,Number(x.present||0)-lost); });
  return perMonth.reduce((s,x)=>s+x,0)/perMonth.length;
}
function matAttSummaryHtml(months,avg){
  const rows=(months||[]).filter(x=>x.hasData).map(x=>`<tr><td>${esc(x.month)}</td><td style="text-align:right;">${x.present}</td><td style="text-align:right;">${x.absent||0}</td><td style="text-align:right;">${Number(x.lateMinutes||0).toFixed(1)}</td><td style="text-align:right;">${Number(x.undertimeMinutes||0).toFixed(1)}</td></tr>`).join("");
  return `<table style="width:100%;font-size:12px;border-collapse:collapse;"><thead><tr style="color:var(--muted);"><th style="text-align:left;">Month</th><th style="text-align:right;">Present</th><th style="text-align:right;">Absent</th><th style="text-align:right;">Late(m)</th><th style="text-align:right;">UT(m)</th></tr></thead><tbody>${rows}</tbody></table>
    <div style="margin-top:6px;font-weight:700;color:var(--green-dark);">Actual avg paid days/month: ${avg!=null?Number(avg).toFixed(2):"—"} <span style="font-weight:400;color:var(--muted);">(present net of late + undertime)</span></div>`;
}
async function matPullAttendance(){
  ...
  try{
    ...
    const avg=matAvgFromAttendance(j.months);
    ...
    box.innerHTML=matAttSummaryHtml(j.months,avg);
    matRecalc();
...
function matCollect(){
  ...
  c.full_pay_basis=g("mat_basis")||"Contractual";
  c.actual_avg_days=(MAT_EDIT&&MAT_EDIT.actual_avg_days)||null; c.attendance=(MAT_EDIT&&MAT_EDIT.attendance)||[];
...
function matRecalc(){
  ...
  box.innerHTML=`
    <div style="font-size:11.5px;color:${r.useActual?'var(--green-dark)':'#8a6a14'};margin-bottom:4px;">${r.useActual?"✓ Actual attendance basis — "+Number(r.effDays).toFixed(2)+" real paid days/month (from PayPlus, net of late/undertime)":"Contractual basis — "+r.effDays+" days/month (theoretical full attendance)"}</div>
    ${row("Full-pay-daily (₱"+Number(r.dr).toLocaleString()+" × "+(r.useActual?Number(r.effDays).toFixed(2):r.effDays)+" ÷ 30)",r.fullPayDaily)}
    ${row("Full-pay maternity benefit (× "+r.days+" days)",r.fpmb,true)}
    <div style="height:8px;"></div>
    ${row("6 highest MSC total"+(r.override?" (overridden)":""),r.sum6)}
    ${row("ADSC (÷ 180)",r.adsc)}
    ${row("SSS maternity benefit (× "+r.days+" days)",r.sssBenefit,true)}
    <div style="height:8px;"></div>
    ${row("Employer salary differential"+(r.employerShare>0?"":" — none (SSS ≥ full pay)"),r.employerShare)}
    ${row("Gross benefit to employee",r.gross,true)}
    ${row("Less deductions",-r.deductions)}
    <div style="display:flex;justify-content:space-between;padding:8px 0 2px;font-weight:800;font-size:16px;color:var(--green-dark);border-top:2px solid var(--green-dark);margin-top:5px;"><span>NET PAYABLE</span><span>${peso(r.net)}</span></div>
    ${r.net<0?'<div class="note" style="background:#fdece9;border-color:#e5b4ae;color:#a4322a;margin-top:6px;">⚠ Net is negative — deductions exceed the benefit. Review.</div>':''}`;
...
async function saveMaternity(){
  ...
  const payload={ employee_id:c.employee_id, employee_name:c.employee_name, category:c.category, worksite:c.worksite||MAT_EDIT.worksite, hire_date:c.hire_date, civil_status:c.civil_status,
    ...
    daily_rate:c.daily_rate, days_per_month:c.days_per_month, full_pay_basis:c.full_pay_basis, actual_avg_days:c.actual_avg_days, attendance:c.attendance, source_period:c.source_period, msc:c.msc, sss_benefit_override:c.sss_benefit_override,
    ...
    full_pay_daily:r.fullPayDaily, fpmb:r.fpmb, sss_benefit:r.sssBenefit, employer_share:r.employerShare, gross_benefit:r.gross, net_payable:r.net,
...
function printMaternity(){
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${esc(c.claim_ref||"Maternity")} — ${esc(c.employee_name||"")}</title><style>
    ...
    <table><tr><th colspan="2">Computation${r.useActual?" · full pay on actual attendance ("+Number(r.effDays).toFixed(2)+" days/mo)":" · full pay on contractual "+r.effDays+" days/mo"}</th></tr>
      <tr><td>Daily salary rate × ${r.useActual?Number(r.effDays).toFixed(2):r.effDays} ÷ 30 = full-pay-daily</td><td class="n">${P(r.fullPayDaily)}</td></tr>
      <tr class="tot"><td>Full-Pay Maternity Benefit (× ${r.days} days)</td><td class="n">${P(r.fpmb)}</td></tr>
      <tr><td>6 highest MSC total</td><td class="n">${P(r.sum6)}</td></tr>
      <tr><td>ADSC (÷ 180)</td><td class="n">${P(r.adsc)}</td></tr>
      <tr class="tot"><td>SSS Maternity Benefit (× ${r.days} days)</td><td class="n">${P(r.sssBenefit)}</td></tr>
      <tr><td>Employer salary differential (if full pay &gt; SSS)</td><td class="n">${P(r.employerShare)}</td></tr>
      <tr class="tot"><td>Gross benefit</td><td class="n">${P(r.gross)}</td></tr>
      <tr><td>Less: deductions during leave</td><td class="n">(${P(r.deductions)})</td></tr>
      <tr class="tot" style="font-size:15px;"><td>NET PAYABLE</td><td class="n">${P(r.net)}</td></tr>
    </table>
    <div class="conf">Maternity benefits are exempt from income tax. Salary data is confidential.</div>
...
function renderPrehire(){
  ...
  const inPipe=PREHIRE.filter(p=>p.phase!=="HIRED"&&p.phase!=="REJECTED"&&p.phase!=="DRAFT");  // DRAFT = agency not-yet-submitted; not in RCC's pipeline
...
function renderOnboarding(){
  ...
  pg.innerHTML=`
    ...
    ${(()=>{ // Uniform send-out list — every hire's size + whether the uniform's been issued
      ...
      const pend=rows.filter(r=>!r.sent).length, needSize=rows.filter(r=>!r.c.uniform_size).length;
      if(!rows.length) return "";
      return `<div class="panel"><h2>Uniform send-out list <span class="count-tag">${pend} to send</span></h2>
        <div class="psub">Every hire's size and whether their uniform's gone out. ${needSize?`<b style="color:var(--red);">${needSize} still need a size recorded</b> (open the case → Uniform).`:"All sizes recorded ✓"} Mark "Issue uniform" done in the case once sent.</div>
        <table><thead><tr><th>Hire</th><th>Worksite</th><th>Position</th><th>Size</th><th>Uniform</th></tr></thead><tbody>
        ${rows.sort((a,b)=>(a.sent?1:0)-(b.sent?1:0)).map(r=>`<tr class="clickable" data-uid="${r.c.id}">
          ...
          <td>${r.c.uniform_size?`<span class="pill di">${esc(r.c.uniform_size)}</span>`:'<span class="pill awol">no size</span>'}</td>
...
function openOnboardingCase(id){
  ...
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:580px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    ...
    <div style="padding:18px 22px 60px;">
      ...
      <div class="panel" style="margin-top:14px;"><h2>Uniform</h2>
        <div class="psub">Size drives the send-out list. Mark the "Issue uniform" task done once it's sent.</div>
        ${sel("onb_usize","Uniform size",["","XS","S","M","L","XL","2XL","3XL"],c.uniform_size||"")}
      </div>
  ...
  m.addEventListener("click",(ev)=>{ if(ev.target===m) m.remove(); });
  const usz=document.getElementById("onb_usize"); if(usz) usz.addEventListener("change",async()=>{ await sb.from("onboarding_cases").update({uniform_size:usz.value||null, updated_at:new Date().toISOString()}).eq("id",c.id); c.uniform_size=usz.value||null; const idx=ONBOARDING.findIndex(x=>x.id===c.id); if(idx>=0) ONBOARDING[idx].uniform_size=usz.value||null; renderOnboarding(); });
...
function collectFinalPay(){ const m=document.getElementById("exModal"); if(!m) return {};
  ...
  const fp={}; FP_ALL.forEach(([k])=>{ fp[k]=num(k); const nt=note(k); if(nt) fp[k+"_note"]=nt; });
  ...
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:600px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    ...
    <div style="padding:18px 22px 60px;">
      ...
      <div class="panel"><h2>Final pay — quitclaim computation ${x.quitclaim_status?qcPill(x.quitclaim_status):""}</h2>
        ...
        <div class="form-grid">
          ${FP_CLAIMS.map(([k,l,h])=>fpField(k,l,h,fp)).join("")}
        </div>
