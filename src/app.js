...
async function loadEmployees(){
  const [emp, br, di, ph, oc, ot, ex, ct, pd, cm, ln, mr, sg, cf, me] = await Promise.all([
...
const REAL_PAGES=new Set(["dashboard","employees","branches","manning","prehire","onboarding","exit","contracts","loans","compliance","settings","signatures","memos"]);
...
function renderManning(){
  ...
  pg.innerHTML=`
    <div class="panel" style="margin-top:0;">
      <h2>Openings <span class="count-tag">${OPENINGS.length} open</span></h2>
...
function newPrehire(){
  ...
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:520px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    ...
    <div style="padding:18px 22px;"><div class="panel" style="margin-top:0;">
      ...
      ${sel("np_worksite","Worksite / location",["Head Office","Warehouse Solaris"].concat([...new Set(BRANCHES.map(b=>b.name))].sort()),e.worksite)}
  ...
  $("#npSave").addEventListener("click",async()=>{
    ...
    const payload={ prehire_id:"PH-"+stamp+"-"+String(rnd).padStart(3,"0"), phase:"APPLIED", full_name:fullname,
      department:v("np_department"), position:v("np_position"), hire_source:"Direct", worksite:v("np_worksite"),
