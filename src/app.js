...
function chargeBody(name,type,amount,ref,date,days,installments){
  ...
  if(inst>1 && !isNaN(raw) && raw>0){
    const per=raw/inst;
    splitLine=`\n\nSHOULD a deduction be found warranted AND you give your written consent, it may be spread over ${inst} pay periods at approximately ₱${per.toLocaleString(undefined,{minimumFractionDigits:2})} per cutoff — never in a single lump — so your take-home pay stays above the minimum wage, in line with DOLE rules on wage deductions.`;
  }
...
function chargeNotice(){
  ...
  const perCut=()=>{ const raw=Number(String(v("chg_amount")||"").replace(/[^0-9.]/g,"")); const n=Math.max(1,parseInt(v("chg_installments")||"1")||1); const el=document.getElementById("chg_perCut"); if(!el) return; el.textContent=(n>1&&raw>0)?`≈ ₱${(raw/n).toLocaleString(undefined,{minimumFractionDigits:2})} per cutoff × ${n} cutoffs`:""; };
