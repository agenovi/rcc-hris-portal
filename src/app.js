...
function computeMaternity(c){
  ...
  const monthly=dr*dpm, fullPayDaily=monthly/30, fpmb=fullPayDaily*days;
  ...
  return { dpm,dr,days,monthly,fullPayDaily,fpmb,msc,sum6,adsc,sssBenefit,employerShare,gross,deductions,net:gross-deductions,override:ov!=null };
}
  ...
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:600px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    ...
    <div id="matForm" style="padding:16px 20px 70px;">
      ...
      <div class="panel"><div class="subhead">C · Salary basis → full pay</div>
        <div class="form-grid">${mf("mat_dr","Daily salary rate (₱)",c.daily_rate)}${mf("mat_dpm","Days / month",c.days_per_month||26)}</div>
        <div class="psub" style="margin:-4px 0 4px;">Full-pay-daily = (rate × days/month) ÷ 30. Days/month basis is company policy — keep it consistent with prior claims.</div>
      </div>
...
function matRecalc(){
  ...
  box.innerHTML=`
    ${row("Full-pay-daily (₱"+Number(r.dr).toLocaleString()+" × "+r.dpm+" ÷ 30)",r.fullPayDaily)}
...
async function saveMaternity(){
  ...
  const payload={ employee_id:c.employee_id, employee_name:c.employee_name, category:c.category, worksite:c.worksite||MAT_EDIT.worksite, hire_date:c.hire_date, civil_status:c.civil_status,
    ...
    daily_rate:c.daily_rate, days_per_month:c.days_per_month, source_period:c.source_period, msc:c.msc, sss_benefit_override:c.sss_benefit_override,
...
function printMaternity(){
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${esc(c.claim_ref||"Maternity")} — ${esc(c.employee_name||"")}</title><style>
    ...
    <table><tr><th colspan="2">Computation</th></tr>
      <tr><td>Daily salary rate × ${r.dpm} ÷ 30 = full-pay-daily</td><td class="n">${P(r.fullPayDaily)}</td></tr>
