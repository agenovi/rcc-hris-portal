...
const ROLE_BY_EMAIL={ "anj@hassarams.com":"admin", "sanjay@hassarams.com":"admin", "hr3@hassarams.com":"payroll", "hr4@hassarams.com":"manager", "hr@hassarams.com":"relations", "richard@hassarams.com":"manager" };  // hr3@=Grazel(payroll) · hr4@=Rhel(HR Manager) · hr@=Juvy(HR Relations) · richard@=IT reviewer (manager view, salary masked)
...
async function loadEmployees(){
  const [emp, br, di, ph, oc, ot, ex, ct, pd, cm, ln, mr, sg, cf, me, evl, clg, scs, mcl, mtg, sysset, apay, npa, pol, pack, proc, mros] = await Promise.all([
...
function renderMovements(){
  ...
  pg.innerHTML=`
    <div class="panel" style="margin-top:0;">
      ...
      <div class="grid kpis" style="grid-template-columns:repeat(3,1fr);">
        <div class="kpi warn"><div class="k-l">Awaiting sign-off</div><div class="k-n">${awaiting}</div><div class="k-s">discretionary NPAs</div></div>
...
// Snapshot the active merchandiser roster onto a meeting (idempotent — skips people already snapshotted).
async function meetingPrepareRoster(date,label){
  if(!date){ alert("Open a meeting first (set its date), then load the expected roster."); return; }
  const have=new Set(meetingRosterFor(date).map(r=>mPersonKey(r.emp_no,r.name)));
  const seen=new Set(have);
  const rows=[];
  (DISERS||[]).filter(d=>(d.status||"").toLowerCase().startsWith("active")).forEach(d=>{
    const k=mPersonKey(d.emp_no,d.name); if(seen.has(k)) return; seen.add(k);
    rows.push({ meeting_date:date, meeting_label:label||null, diser_id:d.id, emp_no:d.emp_no||null,
      name:d.name, store:d.store||null, position:d.position||null, hired_by:d.hired_by||null, expected:true });
  });
  if(!rows.length){ alert("The expected roster for this meeting is already loaded ("+have.size+" merchandisers). Untick anyone not required below."); return; }
  const { error } = await sb.from("meeting_roster").insert(rows);
  if(error){ alert("Couldn't load the roster: "+error.message); return; }
  await reloadMeetings();
}
window.mtgPrepareRoster=(date,label)=>meetingPrepareRoster(date,label);
...
function openExitCase(id){
      ...
      <div class="panel"><h2>Closeout</h2>
        ...
        <div style="border:1px solid var(--line);border-radius:9px;padding:11px 13px;margin:8px 0;background:#f7faf8;">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;">
            ...
            <button class="btn ghost" id="ex_regen_link" type="button" style="color:#9a6a00;">↻ Generate a new link</button>
          </div>
        </div>
  ...
  const rgLink=document.getElementById("ex_regen_link");
  if(rgLink) rgLink.addEventListener("click",async()=>{
    if(!confirm("Generate a NEW link for "+(x.employee_name||"this employee")+"?\n\nThe old link will stop working. Anyone you already sent it to will need the new one.")) return;
    rgLink.disabled=true;
    const tok=(crypto&&crypto.randomUUID)?crypto.randomUUID():null;
    const {error}=await sb.from("exit_clearance").update({emp_link_token:tok, updated_at:new Date().toISOString()}).eq("id",x.id);
    rgLink.disabled=false;
    if(error){ alert(error.message); return; }
    x.emp_link_token=tok;
    const f=document.getElementById("ex_link_field"); if(f){ f.value=exitEmpLink(x); }
    rgLink.textContent="✓ New link ready"; setTimeout(()=>rgLink.textContent="↻ Generate a new link",1800);
  });
