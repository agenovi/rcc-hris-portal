...
// Gov IDs (SSS/PhilHealth/Pag-IBIG/TIN) + bank = visible to ALL logged-in HR (anj call, 2026-07-17). Only SALARY stays payroll-only via canSeePay(). Tighten here if bank ever needs re-locking.
...
// Per-person extra pages on top of their role.
const EXTRA_PAGES_BY_EMAIL={};
...
async function saveEmployee(id,modal){
  ...
  // gov IDs + bank writable by any HR (canSeeIds); SALARY (daily_rate/allowance) still payroll-only (canSeePay)
  if(canSeeIds()){ Object.assign(p,{ sss_number:v("f_sss_number"), philhealth_number:v("f_philhealth_number"), pagibig_number:v("f_pagibig_number"), tin_number:v("f_tin_number"), bank_name:v("f_bank_name"), bank_account_number:v("f_bank_account_number") }); }
  ...
  if(id){
    ...
    if(canSeeIds()){  // bank/gov now editable by any HR — log who changed them
...
async function renderActivity(){
  ...
  pg.innerHTML=`
    ...
    <div class="panel">
      ...
      <div class="psub" style="margin-top:9px;">🔓 Government IDs (SSS/PhilHealth/Pag-IBIG/TIN) &amp; bank details are visible to all HR logins — needed for onboarding &amp; enrolment. Only <b>salary</b> stays restricted to you, Sanjay &amp; Grazel. Every edit to a bank or gov number is logged in the activity feed below.</div>
    </div>
    <div class="panel">
...
function editPrehire(c){
  ...
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:560px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    ...
    <div style="padding:18px 22px;">
      ...
      ${canSeeIds()?`<div class="panel"><div class="subhead">Government numbers &amp; bank</div>
  ...
  document.getElementById("peSave").addEventListener("click",async()=>{
    ...
    if(canSeeIds()){ Object.assign(p,{ sss_number:v("pe_sss"), philhealth_number:v("pe_phil"), pagibig_number:v("pe_pag"), tin_number:v("pe_tin"), bank_name:v("pe_bank"), bank_account_number:v("pe_acct") }); }
...
async function sendFinalPayForSignoff(x,modal){
  ...
  const { data:sig, error:e2 } = await sb.from("signature_requests").insert({
    ...
    body:finalPayBody(x,fp), from_name:sender, amount:net, meta:x.clearance_id,
