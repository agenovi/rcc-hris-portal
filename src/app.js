...
function printMovementNpa(r){
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${esc(r.npa_id||"NPA")} — Notice of Personnel Action Form</title><style>
...
function sigParts(){
  ...
  // For non-Directors, Director-approval items are NOT theirs to sign — show them as
  // "awaiting the Director" instead, so they can see status but can't clear them.
  const pend = isAdmin ? rawPend : rawPend.filter(s=>!isDirectorItem(s));
  const oth  = isAdmin ? rawOth  : rawOth.concat(rawPend.filter(s=>isDirectorItem(s)));
...
async function sendFinalPayForSignoff(x,modal){
  ...
  const { data:sig, error:e2 } = await sb.from("signature_requests").insert({
    ...
    awaiting:"you", with_whom:"Anj Genomal (Director)", status:"pending" }).select().single();
