...
async function loadEmployees(){
  const [emp, br, di, ph, oc, ot, ex, ct, pd, cm, ln, mr, sg, cf, me, evl, clg, scs, mcl, mtg, sysset, apay] = await Promise.all([
...
function tkParse(wb, monthArg){
  ...
  const MON=TK_MONTHS[mi], uaCol=4+mi*3+2, absCol=4+mi*3, lateCol=4+mi;
  const byId={};
  for(let r=10;r<absent.length;r++){ const row=absent[r]; if(!row) continue; const id=row[1],name=row[2],pos=row[3]; if(!name) continue;
    byId[String(id)]={id,name,pos,absent:Number(row[absCol])||0,unauth:Number(row[uaCol])||0,late:0}; }
  for(let r=6;r<late.length;r++){ const row=late[r]; if(!row) continue; const id=String(row[1]),name=row[2]; if(!name) continue;
    if(!byId[id]) byId[id]={id:row[1],name,pos:row[3],absent:0,unauth:0,late:0}; byId[id].late=Number(row[lateCol])||0; }
  const people=Object.values(byId).map(p=>Object.assign({},p,{combined:p.unauth+p.late}));
  const flagged=people.filter(p=>p.combined>=TK_COMBINED_THRESHOLD).sort((a,b)=>b.combined-a.combined);
...
function openingForm(o){
  ...
  m.innerHTML=`<div style="background:#fff;border-radius:14px;max-width:440px;width:100%;padding:22px;">
    ...
    ${sel("op_position","Position *",["Merchandiser","Store Coordinator","Store Supervisor / Lead"],o.position||"Merchandiser")}
