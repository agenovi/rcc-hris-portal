...
function openMovementDrawer(r){
  ...
  const mvCancelBtn=document.getElementById("mvCancel"); if(mvCancelBtn) mvCancelBtn.onclick=async()=>{
    const reason=prompt("Cancel this NPA for "+(r.employee_name||"")+"?\n\nType a reason (e.g. \"Reached end of contract\", \"Duplicate\", \"Wrong rate\"). It stays on record and can be reopened later.");
    ...
    if(error){ alert("Couldn't cancel: "+error.message); mvCancelBtn.disabled=false; mvCancelBtn.textContent="✖ Cancel NPA (e.g. reached EOC)"; return; }
  ...
  const mvReopenBtn=document.getElementById("mvReopen"); if(mvReopenBtn) mvReopenBtn.onclick=async()=>{
    ...
    const back=mvSignedCount(r)>0?"awaiting_signoff":"awaiting_signoff";
