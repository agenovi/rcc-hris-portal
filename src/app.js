...
function openMovementDrawer(r){
  ...
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:600px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    ...
    <div style="padding:16px 20px 70px;">
      <div class="panel" style="margin-top:0;">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;"><h2 style="margin:0;">Movement</h2>${mvStatusPill(r.status)}</div>
      ...
      <div style="display:flex;gap:10px;margin-top:6px;flex-wrap:wrap;">
        <button class="btn ghost" id="mvPrint">View / Print NPA</button>
...
function mvSignStep(r,step){
  ...
  m.innerHTML=`<div style="background:#fff;border-radius:14px;max-width:520px;width:100%;max-height:92vh;overflow-y:auto;padding:22px;">
    ...
    <div style="display:flex;gap:8px;align-items:center;margin-top:6px;flex-wrap:wrap;">
      ...
      <span id="mvSigMsg" style="font-size:12.5px;color:#a4322a;flex:1;"></span>
    </div>
...
// Read-only NPA form + print, signatures stamped where signed (reuses the printNpa layout, incl. the meal row).
function printMovementNpa(r){
  const P=n=>(n==null||n==="")?"":"₱"+Number(n).toLocaleString(undefined,{minimumFractionDigits:2});
  const tot=(dr,al,ml)=>{ if(dr==null&&al==null&&ml==null) return ""; return P((Number(dr)||0)+(Number(al)||0)+(Number(ml)||0)); };
  const payRows=(r.current_daily_rate!=null||r.new_daily_rate!=null||r.current_allowance!=null||r.new_allowance!=null||r.meal_allowance!=null||r.new_meal_allowance!=null);
  const fin=payRows?`
    <h3>III. FINANCIAL DETAILS</h3>
    <table><tr><th></th><th>CURRENT</th><th>NEW</th></tr>
    <tr><td><b>BASIC PAY</b></td><td>${P(r.current_daily_rate)}</td><td>${P(r.new_daily_rate)}</td></tr>
    <tr><td><b>ALLOWANCE</b></td><td>${P(r.current_allowance)}</td><td>${P(r.new_allowance)}</td></tr>
    <tr><td><b>MEAL ALLOWANCE</b></td><td>${P(r.meal_allowance)}</td><td>${P(r.new_meal_allowance)}</td></tr>
    <tr><td><b>TOTAL COMPENSATION</b></td><td>${tot(r.current_daily_rate,r.current_allowance,r.meal_allowance)}</td><td>${tot(r.new_daily_rate,r.new_allowance,r.new_meal_allowance)}</td></tr></table>
    <div class="conf">SALARY data in this form is strictly CONFIDENTIAL.</div>`:"";
  const changeLines=[];
  if(r.new_position) changeLines.push("New position: "+esc(r.new_position));
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${esc(r.npa_id||"NPA")} — Notice of Personnel Action</title><style>
    ...
    h2{text-align:center;font-size:15px;margin:10px 0 14px;} h3{font-size:12.5px;color:#1E3A5F;border-bottom:1px solid #ccd;padding-bottom:2px;margin:16px 0 8px;}
    table{width:100%;border-collapse:collapse;margin:6px 0;} td,th{border:1px solid #bbc;padding:6px 8px;text-align:left;font-size:12.5px;}
    .sr{font-size:10.5px;color:#667;font-weight:700;} .sl{border-bottom:1px solid #333;min-height:40px;margin-bottom:4px;display:flex;align-items:flex-end;} .mut{color:#667;font-size:11.5px;}
    .conf{background:#fdf6e3;border:1px solid #ecdca6;padding:6px 10px;font-size:11.5px;font-weight:600;color:#8a6a14;margin-top:6px;}
    @media print{ body{margin:0 auto;} }
  </style></head><body>
    <div class="lh"><b>ROSHAN COMMERCIAL CORPORATION</b><div>104 Shaw Boulevard, Pasig City</div></div>
    <h2>NOTICE OF PERSONNEL ACTION &nbsp;·&nbsp; ${esc(r.npa_id||"")}</h2>
    <h3>I. EMPLOYEE INFORMATION</h3>
    <table>
      <tr><td><b>Employee Name</b></td><td>${esc(r.employee_name||"—")}</td><td><b>Date Issued</b></td><td>${r.created_at?new Date(r.created_at).toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"}):new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"})}</td></tr>
      <tr><td><b>Employee ID</b></td><td>${esc(r.employee_number||"—")}</td><td><b>Position</b></td><td>${esc(r.current_position||"—")}</td></tr>
      <tr><td><b>Department</b></td><td>${esc(r.department||"—")}</td><td><b>Effective Date</b></td><td>${r.effective_date?fmtDate(r.effective_date):"—"}</td></tr>
    </table>
    <h3>II. REASON FOR PERSONNEL ACTION</h3>
    <div style="margin:3px 0"><b>${esc(MV_ACTION_LABEL[r.action_type]||r.action_type||"")}</b> &nbsp;·&nbsp; Basis: ${esc(basis)}</div>
    ${changeLines.length?`<div style="margin-left:6px;">${changeLines.join("<br>")}</div>`:""}
    ${fin}
...
function renderStoremap(){
  ...
  const vacantScs=[...scSet].filter(sc=>scVacancy(sc).vacant);
  const unassignedStores=BRANCHES.filter(b=>{ const v=scVacancy(b.sc); return v.vacant; });
  pg.innerHTML=`
    <div class="panel" style="margin-top:0;">
      <h2>Store Mapping</h2>
      <div class="psub">Every store mapped to its Store Coordinator (store lead). ${BRANCHES.length} stores · ${open.length} open · ${scSet.size} coordinators${vacantScs.length?` · <span style="color:#a12; font-weight:700;">${vacantScs.length} vacant</span>`:""}</div>
      <div class="grid kpis" style="grid-template-columns:repeat(4,1fr);">
        <div class="kpi"><div class="k-l">Open Stores</div><div class="k-n">${open.length}</div></div>
        <div class="kpi"><div class="k-l">Store Coordinators</div><div class="k-n">${scSet.size}</div></div>
        <div class="kpi ${vacantScs.length?"warn":""}"><div class="k-l">Vacant SC</div><div class="k-n">${vacantScs.length}</div><div class="k-s">stores needing a lead</div></div>
        <div class="kpi"><div class="k-l">Stores w/o a lead</div><div class="k-n">${unassignedStores.filter(b=>b.status==="Open").length}</div></div>
      </div>
  ...
  const paint=()=>{
    ...
    if(view==="sc"){
      ...
      const keys=Object.keys(groups).sort((a,b)=>{
        const va=scVacancy(a).vacant, vb=scVacancy(b).vacant;
        if(va!==vb) return va?-1:1;             // vacant first
        return a==="Unassigned"?-1:b==="Unassigned"?1:a.localeCompare(b);
      });
      body.innerHTML = keys.length ? keys.map(k=>{
        ...
        const badge = v.vacant
          ? `<span class="pill" style="background:#f7dcdc;color:#8a1c1c;border:1px solid #e9b9b9;">⚠ VACANT${v.note?" · "+esc(v.note):""}</span>`
          : `<span class="pill active">Active lead</span>`;
      ...
      const locRow=b=>{ const v=scVacancy(b.sc); return `<tr class="clickable" data-bi="${BRANCHES.indexOf(b)}"><td><b>${esc(b.name)}</b></td>
        <td>${b.sc&&!v.vacant?esc(b.sc):`<span style="color:#a12;font-weight:700;">${esc(b.sc||"Unassigned")} ⚠</span>`}</td>
...
function renderConcerns(){
  ...
  if(!isAdminUser()){ pg.innerHTML=`<div class="panel" style="margin-top:0;"><div class="psub">Concerns &amp; Cases is restricted to the Director/owner.</div></div>`; return; }
    ...
    <div class="panel" style="margin-top:0;">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;">
        <div><h2 style="margin:0;">Concerns &amp; Cases</h2>
          <div class="psub">Arbitration and ongoing legal/labor matters. Director/owner only.</div></div>
  ...
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:620px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    ...
    <div style="padding:16px 20px 70px;">
      <div class="panel" style="margin-top:0;">
        ...
        ${c.doc_url?`<div style="margin-top:8px;"><a class="btn ghost" href="${esc(c.doc_url)}" target="_blank" rel="noopener" style="text-decoration:none;display:inline-block;">Open case file ↗</a></div>`:""}
      </div>
...
function openConcernForm(c){
  ...
  m.innerHTML=`<div style="background:#fff;border-radius:14px;max-width:640px;width:100%;max-height:92vh;overflow-y:auto;padding:22px;">
    ...
    ${fld("Case file link (Drive/Dropbox)",inp("cxfDoc",c.doc_url,"optional URL"))}
  ...
  document.getElementById("cxfSave").onclick=()=>saveConcern(c,isNew);
}
window.openConcernForm=openConcernForm;

async function saveConcern(c,isNew){
...
async function exCreateSignoff(x, name, role, subjectToApproval){
  const nm=(name||"").trim(); if(!nm) return;
  const token=newSignoffToken();
  const label=(x.employee_name||"")+" — exit clearance";
  const doc=exSignoffDoc(x, role)+(subjectToApproval?`<div style="margin-top:10px;font-size:12px;color:#9a6a00;">Note: this signer was chosen manually and is subject to HR's confirmation.</div>`:"");
  const { error } = await sb.from("external_signoffs").insert({ token, context_type:"exit", context_id:String(x.id), context_label:label, signer_name:nm, signer_role:role||null, document_html:doc, created_by:myEmail() });
  if(error){ alert(error.message); return; }
  await loadEmployees(); openExitCase(x.id);
  setTimeout(()=>{ try{ navigator.clipboard.writeText(signoffUrl(token)); }catch(_){} },30);
}
...
function wireExSignoffPanel(x, m){
  const std=m.querySelector("#soAddStd"); const sc=scForExit(x);
  if(std&&sc) std.addEventListener("click",()=>exCreateSignoff(x, sc.name, sc.role, false));
  const cust=m.querySelector("#soAddCustom");
  if(cust) cust.addEventListener("click",()=>{ const n=m.querySelector("#soName").value, r=m.querySelector("#soRole").value; if(!n.trim()){ m.querySelector("#soName").focus(); return; } exCreateSignoff(x, n, r||"Sign-off", true); });
