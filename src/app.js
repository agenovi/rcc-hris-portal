...
function printLoanAgreement(l){
  ...
  // amortization schedule (equal amortization; balance = total payable less cumulative payments)
  let rows=""; const pP=c.amt/c.term, pI=c.interest/c.term;
  for(let m2=1;m2<=c.term;m2++){ const bal=Math.max(0, c.total - c.monthly*m2); rows+=`<tr><td>${m2}</td><td class="num">${P(c.monthly)}</td><td class="num">${P(pP)}</td><td class="num">${P(pI)}</td><td class="num">${P(bal)}</td></tr>`; }
...
function openLoan(id){
  ...
  const loanApproveSign=()=>{
    ...
    captureSignatureModal({
      ...
      subtitle:`${l.applicant_name} · ${loanTypeLabel(l.loan_type)} · ${useTerm} mo${useTerm!==Number(l.term_months||0)?` (originally ${l.term_months||"?"} mo)`:""} · ${loanRateOf()}% p.a. flat`,
