...
function pageAllowed(id){ if(id==='parking') return ((CURRENT_USER&&CURRENT_USER.email)||'').toLowerCase()==='anj@hassarams.com'; if(id==='activity') return isAdminUser(); if(id==='demodata') return isAdminUser(); if(id==='concerns') return canSeeConcerns(); if(id==='maternity') return canSeePay(); if(id==='meetings') return canRunMeetings(); if(id==='movements') return canSeeMovements(); if(id==='govremit') return canEditIds(); if(id==='cosign') return !!CURRENT_USER; if(id==='policies'||id==='processes'||id==='desk'||id==='storemap'||id==='orgchart'||id==='positions'||id==='links') return !!CURRENT_USER; const a=allowedPages(); return !a || a.indexOf(id)!==-1; }
...
function loanCompute(l){
  ...
  return {amt,rate,term,interest,total,monthly,perCutoff:monthly/2};
}
function printLoanAgreement(l){
  ...
  let rows=""; const perPrin=nCut?c.amt/nCut:0, perInt=nCut?c.interest/nCut:0, perPay=nCut?c.total/nCut:0;
  { let cumPay=0;
