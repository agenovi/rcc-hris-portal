...
function applyRoleUI(){
  // Scoped co-signer (Margie): sees EXACTLY one nav item — "Documents to Sign". Hide every other
  // nav item (incl. the always-on policies/desk/links/etc.), all section headers, and the top search.
...
function updateNavBadges(){
  try{
    ...
    if(typeof canSeeIncidents==="function"&&canSeeIncidents()){ navBadgeSet("incidents",(typeof INCIDENTS!=="undefined"?INCIDENTS:[]).filter(i=>typeof incOpen==="function"&&incOpen(i)).length); } else navBadgeSet("incidents",0);
  }catch(_){}
}
...
// ── Payroll cut-off date helpers (cut-offs = the 15th and the LAST DAY of the month). Pure string math
//    (YYYY-MM-DD), built from Y/M/D ints to avoid timezone drift; all guarded → return a "YYYY-MM-DD" string
//    (or "" when the input is unparseable). Function declarations so they're hoisted (no TDZ). ──
function _loanYMD(x){
  if(x instanceof Date){ if(isNaN(x)) return null; return {y:x.getFullYear(),m:x.getMonth()+1,d:x.getDate()}; }
  const s=String(x==null?"":x).slice(0,10), mm=/^(\d{4})-(\d{2})-(\d{2})$/.exec(s);
  if(mm) return {y:+mm[1],m:+mm[2],d:+mm[3]};
  const dt=new Date(x); if(isNaN(dt)) return null; return {y:dt.getFullYear(),m:dt.getMonth()+1,d:dt.getDate()};
}
function lastDayOfMonth(y,m){ return new Date(y,m,0).getDate(); }   // m is 1-12; day 0 of month m+1 = last day of m
function fmtCut(y,m,d){ const p=n=>String(n).padStart(2,"0"); return `${y}-${p(m)}-${p(d)}`; }
function todayCutStr(){ const t=new Date(); return fmtCut(t.getFullYear(),t.getMonth()+1,t.getDate()); }
// the cut-off ON OR AFTER the date: day<=15 → the 15th of that month; else → the last day of that month.
...
// Rush badge + a compact Release-date / First-deduction line for the loan review + release cards.
function loanRushBadgeHtml(l){ return (l&&l.rush)?`<span style="display:inline-block;margin-left:6px;padding:1px 8px;border-radius:9px;background:#fdecea;border:1px solid #f1c9c5;color:#a12622;font-size:10.5px;font-weight:800;vertical-align:middle;">⚡ RUSH</span>`:""; }
function loanDatesLineHtml(l){
  const rd=(l&&l.release_date)?fmtDate(l.release_date):"—", fd=(l&&l.first_deduction_date)?fmtDate(l.first_deduction_date):"—";
  return `<div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;font-size:12px;">
    <span style="padding:3px 9px;border-radius:8px;background:#eef1f5;border:1px solid #dfe4e8;"><b>Release date:</b> ${rd}${loanRushBadgeHtml(l)}</span>
    <span style="padding:3px 9px;border-radius:8px;background:#eef1f5;border:1px solid #dfe4e8;"><b>First deduction:</b> ${fd}</span>
  </div>`;
}
...
function loanCoSignerSigned(s){ return String((s&&s.status)||"").toLowerCase()==="signed"; }
function loanCoSignerDeclined(s){ return String((s&&s.status)||"").toLowerCase()==="declined"; }
function loanCoSignPanelHtml(l){
  const list=loanCoSigners(l);
  if(!list.length) return "";
  const allSigned=list.every(loanCoSignerSigned);
  const chip=s=>loanCoSignerSigned(s)?`<span class="pill active">Signed ✓</span>`:(loanCoSignerDeclined(s)?`<span class="pill closed">Declined</span>`:`<span class="pill probation">Awaiting signature</span>`);
  const peso=(n)=>n==null?"—":"₱"+Number(n).toLocaleString(undefined,{maximumFractionDigits:0});
  const rows=list.map(s=>{
    const signed=loanCoSignerSigned(s), declined=loanCoSignerDeclined(s);
    const canSignNow=!signed && !declined && (myEmail()===String(s.signer_email||"").toLowerCase() || isAdminUser());
    return `<div class="task" style="align-items:flex-start;">
      <div class="dot ${signed?'g':(declined?'r':'a')}" style="margin-top:5px;"></div>
      <div style="flex:1;min-width:0;">
        <div class="tt"><b>${esc(s.signer_role||"")}</b> — ${esc(s.signer_name||"")} ${chip(s)}</div>
        ${signed?`<div class="td" style="color:var(--green);">✓ Signed by ${esc(s.signed_name||s.signer_name||"")}${s.signed_at?" · "+fmtDate(s.signed_at):""}</div>${s.signature_data?`<img src="${esc(s.signature_data)}" alt="signature" style="max-height:64px;margin-top:6px;border:1px solid var(--line);border-radius:6px;background:#fff;">`:""}`:""}
        ${declined&&s.decline_reason?`<div class="td" style="color:var(--red);">Declined: “${esc(s.decline_reason)}”</div>`:""}
        ${(!signed&&!declined)?`<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:6px;">
          <button class="btn ghost" data-lcscopy="${esc(s.token)}" type="button" style="font-size:12px;padding:5px 10px;">🔗 Copy link</button>
          ${canSignNow?`<button class="btn" data-lcssign="${s.id}" type="button" style="font-size:12px;padding:5px 10px;">✍ Co-sign now</button>`:""}
        </div>`:""}
      </div></div>`;
  }).join("");
...
function recordLoanDisbursement(id){
  ...
  const c=(typeof loanCompute==="function")?loanCompute(l):{amt:Number(l.amount||0),rate:loanRateOfType(l),term:Number(l.term_months||0),interest:0,total:Number(l.amount||0),monthly:0,perCutoff:0};
  const revRow=(k,v)=>`<div style="display:flex;justify-content:space-between;gap:10px;padding:4px 0;border-bottom:1px dotted var(--line);font-size:13px;"><span style="color:var(--muted);">${k}</span><span style="font-weight:700;text-align:right;">${v}</span></div>`;
  ...
  m.innerHTML=`<div style="background:#fff;border-radius:14px;max-width:480px;width:100%;max-height:92vh;overflow-y:auto;padding:22px;">
    <div style="font-size:18px;font-weight:800;color:#12352a;margin-bottom:3px;">Record disbursement</div>
    <div class="psub" style="margin-bottom:12px;">${esc(l.applicant_name||"")} · ${esc(l.loan_ref||"")} · ${peso(l.amount)}${loanRushBadgeHtml(l)}</div>
    <div style="background:#f4f8f5;border:1px solid #cfe0d4;border-radius:10px;padding:12px 14px;margin-bottom:12px;">
      <div style="font-weight:800;font-size:13px;color:#12352a;margin-bottom:6px;">Review the figures Accounting is confirming</div>
      ${revRow("Applicant",esc(l.applicant_name||"—"))}
      ${revRow("Loan amount (principal)",peso(c.amt))}
      ${revRow("Interest (flat) · "+esc(loanRatePerMonthLabel(l)),peso(c.interest))}
      ${revRow("Total payable","<b>"+peso(c.total)+"</b>")}
      ${revRow("Term",(c.term||"—")+" month(s)")}
      ${revRow("Monthly amortization",peso(c.monthly))}
      ${revRow("Per cut-off",peso(c.perCutoff))}
      ${revRow("Release date",(l.release_date?fmtDate(l.release_date):"—")+(l.rush?" · ⚡ RUSH":""))}
      ${revRow("First deduction",l.first_deduction_date?fmtDate(l.first_deduction_date):"—")}
    </div>
    <label style="display:flex;gap:9px;align-items:flex-start;font-size:13px;cursor:pointer;margin-bottom:12px;"><input type="checkbox" id="lrpChk" style="margin-top:2px;"> <span>I've reviewed these figures — they're correct.</span></label>
    ...
    <div style="display:flex;gap:10px;margin-top:16px;">
      ...
      <button class="btn" id="lrpSave" type="button" disabled style="opacity:.55;">Save proof</button>
    </div></div>`;
  m.addEventListener("click",ev=>{ if(ev.target===m) m.remove(); });
  document.getElementById("lrpCancel").onclick=()=>m.remove();
  const lrpChk=document.getElementById("lrpChk"), lrpSaveBtn=document.getElementById("lrpSave");
  lrpChk.addEventListener("change",()=>{ const ok=lrpChk.checked; lrpSaveBtn.disabled=!ok; lrpSaveBtn.style.opacity=ok?"":".55"; });
  lrpSaveBtn.onclick=async()=>{
    if(lrpSaveBtn.disabled) return;
    if(!lrpChk.checked){ alert("Tick “I've reviewed these figures — they're correct.” to confirm before saving."); return; }
    ...
    const btn=lrpSaveBtn; btn.disabled=true; btn.textContent="Uploading…";
    const f=fi.files[0];
    ...
    // No new column for the review: stamp the confirmation into hr_notes (Accounting's attestation).
    const confirmNote=`[Accounting reviewed & confirmed figures · ${myName()||myEmail()||"Accounting"} · ${fmtDate(nowIso)}]`;
    const newNotes=(l.hr_notes?l.hr_notes+"\n":"")+confirmNote;
    const {error}=await sb.from("loans").update({ release_proof:{path:path,name:f.name,ref:ref||null}, release_ref:ref||null, hr_notes:newNotes, updated_at:nowIso }).eq("id",l.id);
...
function loanCompute(l){
  const baseRate=(typeof LOAN_RATES!=="undefined"?LOAN_RATES[l.loan_type]:null)??12, term=Number(l.term_months||0), amt=Number(l.amount||0);
  // Educational: full tuition is the loan/assistance amount, but the PARENT amortizes only their 50% (0% interest)
  // over the loan term. The child's 50% is a deferred obligation (3 months post-graduation) — not on this schedule.
  const edu=l.loan_type==="educational";
  const rate=edu?0:baseRate;
  const amortPrincipal=edu?amt*0.5:amt;
  const interest=amortPrincipal*(rate/100)*(term/12), total=amortPrincipal+interest, monthly=term?total/term:0;
  return {amt,rate,term,interest,total,monthly,perCutoff:monthly/2,amortPrincipal,edu};
}
function printLoanAgreement(l){
  if(!l.amount||!l.term_months){ alert("This loan needs an amount and a term to compute."); return; }
  const w=window.open("","_blank"); if(!w){ alert("Allow pop-ups to download the agreement."); return; }
  const c=loanCompute(l);
  const P=n=>"₱"+Number(n||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2});
  const E=s=>String(s==null?"":s).replace(/[&<>]/g,x=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[x]));
  const moto=l.loan_type==="moto";
  // Bi-monthly amortization schedule keyed to the real payroll cut-offs (15th ↔ EOM) starting from the loan's
  ...
  { let cumPay=0;
    for(let k=1;k<=nCut;k++){
      const isLast=(k===nCut);
      const pay=isLast?Math.max(0, c.total-cumPay):Math.round(perPay*100)/100; cumPay+=pay;
      const bal=Math.max(0, Math.round((c.total-cumPay)*100)/100);
      const dcell=schedDates.length?E(fmtDate(schedDates[k-1])):("Cut-off "+k);
      rows+=`<tr><td>${k}</td><td>${dcell}</td><td class="num">${P(pay)}</td><td class="num">${P(perPrin)}</td><td class="num">${P(perInt)}</td><td class="num">${P(bal)}</td></tr>`;
    }
  }
  const loanPeriodStr=(schedDates.length)?(E(fmtDate(schedDates[0]))+" – "+E(fmtDate(schedDates[schedDates.length-1]))):"";
  ...
  const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${E(l.loan_ref||"Loan")} — Loan Agreement</title><style>
    *{box-sizing:border-box}body{font-family:'Segoe UI',Arial,Helvetica,sans-serif;color:#1a1a1a;font-size:12.5px;line-height:1.5;margin:28px;background:#f4f5f7;}
    ...
    <table><tbody>
      ...
      <tr><td>Interest rate</td><td class="num">${E(loanRatePerMonthLabel(l))} (${c.rate}% per annum, flat)</td></tr>
...
function loanSummaryStripHtml(l){
  ...
  return `<div style="display:flex;flex-wrap:wrap;gap:8px;margin:0 0 12px;">
    ...
    ${cell("Interest",loanRatePerMonthLabel(l)+" <span style='font-weight:600;font-size:11px;'>flat</span>")}
...
function openLoan(id){
  ...
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:540px;height:100%;overflow-y:auto;">
    ...
    <div style="padding:18px 22px 60px;">
      ...
      <div class="panel">
        ...
        ${(l.release_date||l.first_deduction_date||l.status==="Approved"||l.status==="Released")?loanDatesLineHtml(l):""}
        ${(isAdminUser()&&(l.status==="Approved"||l.status==="Released"||l.release_date))?`<div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;">
          ${l.rush
            ? `<button class="btn ghost" id="loanUnrush" style="font-size:12px;padding:5px 10px;color:#1f6b3a;border-color:#cfe6d6;">↩ Back to cut-off schedule</button>`
            : `<button class="btn ghost" id="loanRush" style="font-size:12px;padding:5px 10px;color:#a12622;border-color:#f1c9c5;">⚡ Rush release (off-cycle)</button>`}
        </div>`:""}
  ...
  function updateLoanAdj(){
    ...
    out.innerHTML=(disposable
      ? `This amount → <b>${P(mon)}/mo</b> at ${loanRatePerMonthLabel(l)} flat · combined ${P(combined)} vs 15% guide ${P(guide)} `
  ...
  const loanSendUp=()=>{
    ...
    mm.innerHTML=`<div style="background:#fff;border-radius:14px;max-width:480px;width:100%;padding:22px;">
      ...
      <div class="psub" style="margin-bottom:10px;">Name the supervisor who is <b>aware</b> of this loan — <b>required</b>, so it's on record that a supervisor knows. No other details needed. This does <b>not</b> block approval.</div>
      ...
      <input id="suName" list="suEmpList" placeholder="Supervisor name (required)" style="width:100%;padding:9px 11px;border:1px solid var(--line);border-radius:8px;margin-bottom:8px;" value="${esc(l.supervisor_name||"")}">
    ...
    document.getElementById("suSend").onclick=()=>{
      ...
      if(!nm){ alert("Please name the supervisor who is aware of this loan before sending it up to Management."); return; }
      const em2=(document.getElementById("suEmail").value||"").trim().toLowerCase();
      const patch={ status:"Management", supervisor_name:nm, supervisor_email:em2||null, supervisor_status:"pending" };
  ...
  // Rush release (admin only): release off-cycle TODAY; first deduction = the cut-off strictly after today.
  const rushBtn=document.getElementById("loanRush");
  if(rushBtn) rushBtn.addEventListener("click",async()=>{
    if(!confirm("Rush release (off-cycle)?\n\nThis sets the release date to TODAY and starts the first salary deduction at the next cut-off after today.")) return;
    rushBtn.disabled=true; rushBtn.textContent="Saving…";
    const today=todayCutStr(), fd=cutoffAfter(today), nowIso=new Date().toISOString();
    const {error}=await sb.from("loans").update({rush:true,release_date:today,first_deduction_date:fd,updated_at:nowIso}).eq("id",l.id);
    if(error){ alert(error.message); rushBtn.disabled=false; rushBtn.textContent="⚡ Rush release (off-cycle)"; return; }
    await logChange("loan",l.id,l.applicant_name,"Rush release (off-cycle)",l.loan_ref+" · release "+today);
    await loadEmployees(); m.remove(); openLoan(l.id);
  });
  const unrushBtn=document.getElementById("loanUnrush");
  if(unrushBtn) unrushBtn.addEventListener("click",async()=>{
    if(!confirm("Back to the cut-off schedule?\n\nThis re-dates the release to the next payroll cut-off (15th / end-of-month).")) return;
    unrushBtn.disabled=true; unrushBtn.textContent="Saving…";
    const rd=nextCutoff(new Date()), fd=cutoffAfter(rd), nowIso=new Date().toISOString();
    const {error}=await sb.from("loans").update({rush:false,release_date:rd,first_deduction_date:fd,updated_at:nowIso}).eq("id",l.id);
    if(error){ alert(error.message); unrushBtn.disabled=false; unrushBtn.textContent="↩ Back to cut-off schedule"; return; }
    await logChange("loan",l.id,l.applicant_name,"Back to cut-off schedule",l.loan_ref+" · release "+rd);
    await loadEmployees(); m.remove(); openLoan(l.id);
  });
  ...
  // ── Loan co-signing chain (forward → Grazel in-portal / Margie via link) ──
  const fwd=document.getElementById("loanFwdCosign");
  if(fwd) fwd.addEventListener("click",async()=>{
    ...
    const rows=LOAN_COSIGNERS.map(cs=>({
      token:newSignoffToken(), context_type:"loan", context_id:String(l.id),
      context_label:`Loan ${l.loan_ref} · ${l.applicant_name} · ${peso(l.amount)}`,
      signer_name:cs.name, signer_role:cs.role, signer_email:cs.email,
      document_html:docHtml, status:"pending", created_by:myEmail() }));
    const {error}=await sb.from("external_signoffs").insert(rows);
    if(error){ alert(error.message); fwd.disabled=false; fwd.textContent="Forward for co-signing (Juvy sends)"; return; }
    await sb.from("loans").update({ forwarded_by: myName()||myEmail(), forwarded_at: new Date().toISOString() }).eq("id",l.id);
    await logChange("loan",l.id,l.applicant_name,"Forwarded for co-signing",l.loan_ref);
    // TODO email Grazel + Margie once Resend key is set
    await loadEmployees(); m.remove(); openLoan(l.id);
  });
  m.querySelectorAll("[data-lcscopy]").forEach(b=>b.addEventListener("click",async()=>{
    const url=signoffUrl(b.dataset.lcscopy);
    try{ await navigator.clipboard.writeText(url); b.textContent="✓ Copied"; setTimeout(()=>b.textContent="🔗 Copy link",1600); }catch(_){ prompt("Copy the co-signing link:", url); }
  }));
  m.querySelectorAll("[data-lcssign]").forEach(b=>b.addEventListener("click",()=>{
...
/* ===== Reliever coverage — a person covering >1 store is a reliever (auto-badge). Tag by adding the store(s) they relieve. ===== */
async function loadRelieverCoverage(empId){ if(!empId) return []; try{ const {data}=await sb.from("reliever_coverage").select("*").eq("employee_id", String(empId)).order("from_date",{ascending:false}); return data||[]; }catch(_){ return []; } }
function relieverIsActive(r){ const t=new Date().toISOString().slice(0,10); return !r.to_date || r.to_date>=t; }
async function renderRelieverBox(e){
  const box=document.getElementById("relieverBox"); const badge=document.getElementById("relieverBadge");
  if(!box) return;
  const rows=await loadRelieverCoverage(e.employee_id);
  const act=rows.filter(relieverIsActive);
  if(badge) badge.innerHTML = act.length ? `<span class="pill" style="background:#efe7fb;color:#5b3a9e;border:1px solid #d8c8f0;margin-left:6px;">🔁 Reliever · ${act.length} store${act.length>1?"s":""}</span>` : "";
  const canEdit=canManageStores();
  const stores=[...new Set((BRANCHES||[]).filter(b=>b.status==="Open").map(b=>b.name))].sort();
  const today=new Date().toISOString().slice(0,10);
  const rowHtml=rows.map(r=>{ const ended=r.to_date && r.to_date<today;
    return `<div class="efield"><div class="el">${esc(r.cover_store)}</div><div class="ev">${r.from_date?fmtDate(r.from_date):"—"} → ${r.to_date?fmtDate(r.to_date):"ongoing"}${ended?' <span class="note" style="display:inline;">ended</span>':""}${r.created_by?` <span class="note" style="display:inline;">added by ${esc(r.created_by)}</span>`:""}${canEdit?` <a href="#" data-rc-del="${r.id}" style="color:#a12;margin-left:8px;">remove</a>`:""}</div></div>`; }).join("");
  box.innerHTML=`<div class="panel">
    <div><h2 style="margin:0;">🔁 Reliever coverage</h2>
      <div class="psub" style="margin:2px 0 4px;">Someone covering more than their home store is a <b>reliever</b>. Tag them by adding the store(s) they relieve — the badge is automatic. Home store <b>${esc(e.worksite||"—")}</b> follows PayPlus and stays. Every add/remove is logged.</div></div>
    ${rows.length?rowHtml:`<div class="psub" style="margin-top:4px;">Not a reliever — single store.${canEdit?" Add a covering store below to tag as reliever.":""}</div>`}
    ${canEdit?`<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end;margin-top:10px;">
      <div style="flex:2;min-width:150px;"><label style="font-size:11px;color:#6a766f;font-weight:700;display:block;">Covering store</label><input list="rcStores" id="rc_store" placeholder="Store…" style="width:100%;padding:8px 10px;border:1px solid #e2e7e4;border-radius:8px;font-size:13px;"><datalist id="rcStores">${stores.map(s=>`<option value="${esc(s)}"></option>`).join("")}</datalist></div>
      <div><label style="font-size:11px;color:#6a766f;font-weight:700;display:block;">From</label><input type="date" id="rc_from" style="padding:7px 9px;border:1px solid #e2e7e4;border-radius:8px;font-size:13px;"></div>
      <div><label style="font-size:11px;color:#6a766f;font-weight:700;display:block;">To (blank=ongoing)</label><input type="date" id="rc_to" style="padding:7px 9px;border:1px solid #e2e7e4;border-radius:8px;font-size:13px;"></div>
      <button class="btn" id="rc_add">＋ Add store</button>
    </div>`:""}</div>`;
  const addBtn=document.getElementById("rc_add"); if(addBtn) addBtn.addEventListener("click",()=>relieverAdd(e));
  box.querySelectorAll("[data-rc-del]").forEach(a=>a.addEventListener("click",(ev)=>{ ev.preventDefault(); relieverDel(a.dataset.rcDel, e); }));
}
async function relieverAdd(e){
  const store=(document.getElementById("rc_store").value||"").trim(); if(!store){ alert("Pick the covering store."); return; }
  if((e.worksite||"").trim()===store){ alert("That's the home store — add a DIFFERENT store they cover."); return; }
  const from=document.getElementById("rc_from").value||null, to=document.getElementById("rc_to").value||null;
  const {error}=await sb.from("reliever_coverage").insert({ employee_id:e.employee_id||null, employee_name:e.full_name, home_store:e.worksite||null, cover_store:store, from_date:from, to_date:to, created_by:(CURRENT_USER&&CURRENT_USER.email)||null });
  if(error){ alert("Couldn't add: "+error.message); return; }
  try{ await logChange("reliever", e.employee_id||null, e.full_name, "Reliever store added", store+(from?(" from "+from):"")+(to?(" to "+to):"")); }catch(_){}
  renderRelieverBox(e);
}
async function relieverDel(id,e){
  if(!confirm("Remove this covering store?")) return;
  const {error}=await sb.from("reliever_coverage").delete().eq("id", id);
  if(error){ alert(error.message); return; }
  try{ await logChange("reliever", e.employee_id||null, e.full_name, "Reliever store removed", ""); }catch(_){}
  renderRelieverBox(e);
}
  ...
  pg.innerHTML=`
    ...
    <div class="panel" style="margin-top:0;">
      <div class="emp-head">
        ...
        ${typePill(e)} ${statusPill(e.status)}<span id="relieverBadge"></span>
...
let MV_FILTER=null;
let MV_MISSING_ONLY=false;   // when true, table shows only pay NPAs with NO staff email on file
...
function renderMovements(){
  ...
  // Effective-date change = owner only (anj). Court/wage-order dates move; signatures stay intact.
  const canEditEff=["anj@hassarams.com","claude.test@hassarams.com"].includes(meEmail);
  ...
  const batchCardsHtml=batchArr.map(b=>{ const rows=b.rows, tot=rows.length;
    ...
    return `<div class="mvbatch" data-bk="${esc(b.key)}" style="cursor:pointer;border:1px solid ${sel?"#2f9e5f":"#e2ebe5"};background:${sel?"#f2faf5":"#fff"};border-radius:11px;padding:12px 14px;margin-bottom:8px;">
      ...
      <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;font-size:12.5px;">
        ...
        <div style="color:#12352a;">${step}</div></div>
      ${canEditEff?`<div style="margin-top:9px;border-top:1px dashed #e2ebe5;padding-top:8px;"><button class="btn ghost mvEditEff" data-bk="${esc(b.key)}" data-eff="${esc(eff||"")}" style="font-size:12px;padding:4px 11px;">📅 Change effective date</button> <span class="note" style="display:inline;">moves all ${tot} — signatures stay</span></div>`:""}
      </div>`; }).join("");
  ...
  // Email coverage — how many NPAs have a staff email on file (toward auto-emailing them).
  const payN=R.filter(r=>r.status!=="cancelled"&&(r.current_daily_rate!=null||r.new_daily_rate!=null||r.current_allowance!=null||r.new_allowance!=null));
  const emailN=payN.filter(r=>String(r.notify_email||"").trim()).length;
  ...
  pg.innerHTML=`
    <div class="panel" style="margin-top:0;">
      ...
      <div class="actionbar">
        ...
        ${canSeePay()&&payN.length?`<button class="btn ghost" id="mvEmailAll">✉ Email staff their NPA</button>`:""}
      </div>
      ...
      ${mvCatLine?`<div class="psub" style="margin:2px 0 6px;padding:8px 12px;background:#f4f7f5;border:1px solid #e2ebe5;border-radius:9px;"><b>By group:</b> ${mvCatLine} &nbsp;·&nbsp; <b>${Object.values(mvCatCounts).reduce((a,b)=>a+b,0)}</b> total &nbsp;·&nbsp; 📧 <b>${emailN}</b>/${payN.length} have a staff email${emailN<payN.length?` · <a href="#" id="mvShowMissing" style="color:#9a6a00;font-weight:700;">${MV_MISSING_ONLY?"↺ show all":"show the "+(payN.length-emailN)+" missing →"}</a>`:" — all set for auto-email"}</div>`:""}
  ...
  const bSM=$("#mvShowMissing"); if(bSM) bSM.addEventListener("click",e=>{ e.preventDefault(); MV_MISSING_ONLY=!MV_MISSING_ONLY; renderMovements(); });
  const bEA=$("#mvEmailAll"); if(bEA) bEA.addEventListener("click",()=>mvEmailBatch(R));
  ...
  // Owner-only: change the effective date for a whole batch (court/wage-order date moved) — signatures untouched.
  $$("#page-movements .mvEditEff").forEach(el=>el.addEventListener("click",ev=>{ ev.stopPropagation(); mvChangeEffDate(el.dataset.bk, el.dataset.eff||""); }));
...
// Owner-only: move the effective date of an entire batch (e.g. wage order deferred by a court TRO).
// ONLY effective_date changes — signatures already on the NPAs are left exactly as-is (no re-signing).
function mvChangeEffDate(batchKey, curEff){
  const rows=(NPAS||[]).filter(r=>mvBatchKey(r)===batchKey);
  if(!rows.length){ alert("No movements found for this batch."); return; }
  const label=mvBatchLabel(rows[0]);
  const signedN=rows.filter(r=>mvSignedCount(r)>0).length;
  let m=document.getElementById("mvEffModal"); if(!m){ m=document.createElement("div"); m.id="mvEffModal"; document.body.appendChild(m); }
  m.style.cssText="position:fixed;inset:0;z-index:9999;background:rgba(14,50,25,.45);display:flex;align-items:center;justify-content:center;padding:24px;";
  m.innerHTML=`<div style="background:#fff;border-radius:14px;max-width:480px;width:100%;padding:22px;max-height:90vh;overflow-y:auto;">
    <h2 style="font-size:17px;color:var(--green-dark);margin-bottom:2px;">Change effective date</h2>
    <div class="psub" style="margin-bottom:10px;">${esc(label)} · <b>${rows.length}</b> staff${curEff?` · currently effective <b>${fmtDate(curEff)}</b>`:""}.</div>
    <div class="note" style="margin:0 0 12px;background:#eef7f0;border-color:#cfe6d6;color:#12352a;">Only the <b>effective date</b> changes. ${signedN?`All <b>${signedN}</b> signatures already on these NPAs stay exactly as-is — nobody re-signs.`:"Signatures already on these NPAs stay exactly as-is."} The change (old → new date, who, when, and your reason) is logged.</div>
    <label style="display:block;font-size:11px;font-weight:700;color:#6a766f;text-transform:uppercase;margin-bottom:4px;">New effective date *</label>
    <input type="date" id="mvEffNew" value="${esc(curEff||"")}" style="width:100%;padding:10px 12px;border:1px solid #e2e7e4;border-radius:8px;font-size:14px;margin-bottom:12px;">
    <label style="display:block;font-size:11px;font-weight:700;color:#6a766f;text-transform:uppercase;margin-bottom:4px;">Reason for the change *</label>
    <textarea id="mvEffReason" rows="3" style="width:100%;padding:9px 11px;border:1px solid #e2e7e4;border-radius:8px;font-size:13.5px;">Implementation deferred per Pasig RTC TRO on Wage Order NCR-27; effective date moved to align with the wage order's lawful implementation date.</textarea>
    <div id="mvEffMsg" style="font-size:13px;color:#a4322a;margin:6px 0;"></div>
    <div style="display:flex;gap:10px;margin-top:6px;"><button class="btn ghost" id="mvEffCancel" style="flex:1;">Cancel</button><button class="btn" id="mvEffGo" style="flex:1;">Change date for all ${rows.length}</button></div>
  </div>`;
  m.addEventListener("click",ev=>{ if(ev.target===m) m.remove(); });
  document.getElementById("mvEffCancel").onclick=()=>m.remove();
  document.getElementById("mvEffGo").onclick=async()=>{
    const nd=(document.getElementById("mvEffNew").value||"").trim();
    const reason=(document.getElementById("mvEffReason").value||"").trim();
    const msg=document.getElementById("mvEffMsg");
    if(!/^\d{4}-\d{2}-\d{2}$/.test(nd)){ msg.textContent="Pick a valid date."; return; }
    if(!reason){ msg.textContent="A reason is required — it goes on the audit log."; return; }
    if(nd===curEff){ msg.textContent="That's the same date it already has."; return; }
    const btn=document.getElementById("mvEffGo"); btn.disabled=true; btn.textContent="Changing…";
    const ids=rows.map(r=>r.id);
    const { error }=await sb.from("personnel_actions").update({ effective_date:nd, updated_at:new Date().toISOString() }).in("id",ids);
    if(error){ msg.textContent="Couldn't change: "+error.message; btn.disabled=false; btn.textContent="Change date for all "+rows.length; return; }
    await logChange("movement",null,label,"Effective date changed",rows.length+" NPA(s) · "+(curEff||"—")+" → "+nd+" · "+reason);
    m.remove(); await loadEmployees(); window.go("movements");
  };
}
window.mvChangeEffDate=mvChangeEffDate;
...
function openMovementDrawer(r){
  ...
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:600px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    ...
    <div style="padding:16px 20px 70px;">
      <div class="panel" style="margin-top:0;">
        ...
        <div style="margin-top:10px;padding:9px 11px;background:#f4f7f5;border:1px solid #e2ebe5;border-radius:9px;">
          <div style="font-size:11px;font-weight:800;letter-spacing:.6px;color:#6B7785;margin-bottom:4px;">📧 STAFF EMAIL (for sending them this NPA)</div>
          <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
            <input id="mvEmail" value="${esc(r.notify_email||"")}" placeholder="name@example.com" autocomplete="off" style="flex:1;min-width:190px;padding:7px 9px;border:1px solid #dfe5e1;border-radius:7px;font-size:13px;">
            <button class="btn ghost" id="mvSaveEmail" style="padding:7px 12px;">Save email</button>
            <span id="mvEmailMsg" class="note" style="display:inline;"></span>
          </div>
          <div class="note" style="display:inline;font-size:11.5px;margin-top:4px;">${r.notified_at?`✓ emailed ${fmtDate(r.notified_at)}`:"Not yet emailed — one-click auto-send to staff turns on once the email key is set."}</div>
        </div>
  ...
  const mvSaveEmailBtn=document.getElementById("mvSaveEmail"); if(mvSaveEmailBtn) mvSaveEmailBtn.onclick=async()=>{
    const val=((document.getElementById("mvEmail")||{}).value||"").trim();
    const msg=document.getElementById("mvEmailMsg");
    if(val && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)){ msg.textContent="That doesn't look like an email."; msg.style.color="#c0392b"; return; }
    mvSaveEmailBtn.disabled=true; mvSaveEmailBtn.textContent="Saving…";
    const { error }=await sb.from("personnel_actions").update({ notify_email:val||null, notify_employee:!!val, updated_at:new Date().toISOString() }).eq("id",r.id);
    if(error){ msg.textContent=error.message; msg.style.color="#c0392b"; mvSaveEmailBtn.disabled=false; mvSaveEmailBtn.textContent="Save email"; return; }
    r.notify_email=val||null; r.notify_employee=!!val; msg.textContent="Saved ✓"; msg.style.color="#1f6b3a"; mvSaveEmailBtn.disabled=false; mvSaveEmailBtn.textContent="Save email";
  };
...
// Batch email: send each staff member their NPA PDF. Sender = Grazel (paymaster). Actual send
// is gated behind the Resend key + hassarams.com domain check — until then the button is disabled.
function mvEmailBatch(R){
  const payN=(R||[]).filter(r=>r.status!=="cancelled" && (r.current_daily_rate!=null||r.new_daily_rate!=null||r.current_allowance!=null||r.new_allowance!=null));
  const ready=payN.filter(r=>String(r.notify_email||"").trim());
  const missing=payN.filter(r=>!String(r.notify_email||"").trim());
  const alreadySent=ready.filter(r=>r.notified_at).length;
  let m=document.getElementById("mvEmailModal"); if(!m){ m=document.createElement("div"); m.id="mvEmailModal"; document.body.appendChild(m); }
  m.style.cssText="position:fixed;inset:0;z-index:10001;background:rgba(14,30,50,.55);display:flex;align-items:center;justify-content:center;padding:20px;";
  m.innerHTML=`<div style="background:#fff;border-radius:14px;max-width:540px;width:100%;max-height:90vh;overflow-y:auto;padding:22px;">
    <div style="font-size:10.5px;font-weight:800;letter-spacing:1.6px;color:#6B7785;">EMAIL NPA TO STAFF · BATCH</div>
    <div style="font-size:18px;font-weight:800;color:#12352a;margin:2px 0 8px;">Send each employee their NPA</div>
    <div class="psub" style="margin-bottom:8px;">Emails the RCC-letterhead NPA PDF to each staff member and stamps “✓ emailed” on their record.</div>
    <div style="background:#f4f7f5;border:1px solid #e2ebe5;border-radius:9px;padding:10px 12px;font-size:13px;">
      <div><b>From:</b> Grazel Lyn Agulto — RCC HR &lt;hr3@hassarams.com&gt;</div>
      <div style="margin-top:4px;"><b>${ready.length}</b> ready to email${alreadySent?` <span class="note" style="display:inline;">(${alreadySent} already sent)</span>`:""} &nbsp;·&nbsp; <b style="color:#9a6a00;">${missing.length}</b> skipped — no email</div>
    </div>
    ${missing.length?`<div style="margin-top:10px;"><div style="font-weight:700;color:#9a6a00;font-size:12.5px;margin-bottom:4px;">Won't be emailed — no address on file (fill these first):</div>
      <div style="max-height:150px;overflow-y:auto;border:1px solid #f0e2b8;border-radius:8px;padding:6px 10px;font-size:12.5px;background:#fffdf6;">${missing.map(r=>esc(r.employee_name||"—")+(r.employee_number?" · "+esc(String(r.employee_number)):"")).join("<br>")}</div>
      <button class="btn ghost" id="mvEmailShowMissing" style="margin-top:6px;">Show these in the list so I can fill them</button></div>`:`<div class="note" style="margin-top:10px;background:#eef7f0;border-color:#cfe6d6;color:#12352a;">✓ Everyone has an email — no one will be skipped.</div>`}
    <div class="note" style="margin-top:12px;background:#fff8e6;border-color:#f0e2b8;color:#6b5a17;">⚠ <b>Sending isn't switched on yet.</b> To email staff as <b>hr3@hassarams.com</b> (Grazel), two one-time steps: (1) the free <b>Resend key</b> (waiting item #16) and (2) a quick <b>domain check</b> on hassarams.com. Once both are in, this button sends all ${ready.length} at once and marks each “emailed”.</div>
    <div style="display:flex;gap:10px;margin-top:14px;">
      <button class="btn ghost" id="mvEmailClose" style="margin-left:auto;">Close</button>
      <button class="btn" id="mvEmailSend" disabled title="Turns on once the Resend key + domain are set" style="opacity:.55;cursor:not-allowed;">✉ Send all ${ready.length} — setup needed</button>
    </div></div>`;
  m.addEventListener("click",ev=>{ if(ev.target===m) m.remove(); });
  document.getElementById("mvEmailClose").onclick=()=>m.remove();
  const sm=document.getElementById("mvEmailShowMissing"); if(sm) sm.onclick=()=>{ m.remove(); MV_MISSING_ONLY=true; renderMovements(); };
}
window.mvEmailBatch=mvEmailBatch;
...
const CASE_STATUSES=["Open","Pending","Ongoing","For settlement/payment","Settled","Awarded","Dismissed","Withdrawn","Closed","Archived"];
function concernStatusPill(s){
  const map={ "Open":"warn", "Pending":"warn", "Ongoing":"warn", "For settlement/payment":"info", "Settled":"active", "Awarded":"danger", "Dismissed":"", "Withdrawn":"", "Closed":"", "Archived":"" };
  const cls=map[s]!=null?map[s]:"";
  const bg={ warn:"#fdf0d5;color:#8a6a14;border:1px solid #ecd9a6", info:"#e7edf6;color:#28425f;border:1px solid #c6d5e6", active:"#e4f3ea;color:#155e3f;border:1px solid #bfe0cc", danger:"#f7dcdc;color:#8a1c1c;border:1px solid #e9b9b9", "":"#eceff1;color:#55606a;border:1px solid #d9dee2" }[cls];
...
function activeCases(){ return CONCERNS.filter(c=>["Open","Pending","Ongoing","For settlement/payment"].includes(c.status)); }
...
/* ================= INCIDENT REPORTS — HO & Disers incident log =================
   Operational incident tracker (from HO & DISERS INCIDENT REPORT MONITORING).
   Logs each incident, whether an NTE was issued, status, repeat-offense count,
   action taken and resolution. Visible to admin + HR manager/relations/payroll. */
var INCIDENTS = (typeof INCIDENTS!=="undefined" && INCIDENTS) || [];
const INCIDENT_TYPES=["ATTENDANCE","LATE OPENING/EARLY CLOSING","LATE ENTRY IN POS/STOREHUB","WRONG AMOUNT PUNCH IN","WRONG CODE ENTER","DOUBLE PUNCH ITEM","WRONG ITEM SERVE","DISCOUNT; MARKED UP PRICE","UNDECLARED ITEM-MANUAL INVOICE","CASH DEPOSIT DISCREPANCIES","QUALITY/ICC","ALLEGED WORKPLACE HARASSMENT","ACCIDENT","THEFT","OTHER"];
const INCIDENT_STATUSES=["PENDING","ONGOING","DONE","CLOSE"];
const INCIDENT_COUNTS=["FIRST","SECOND","THIRD","FOURTH+"];
const INCIDENT_CHANNELS=["EMAIL","DIRECT SUBMIT OF IR","PHONE","WALK-IN","OTHER"];
const INCIDENT_OFFICE_TYPES=["OUTLET STORE","DEPARTMENT STORE","SOLARIS WH","HEAD OFFICE"];
function canSeeIncidents(){ const r=userRole(); return r==="admin"||r==="manager"||r==="relations"||r==="payroll"; }
function incStatusPill(s){ s=(s||"").toUpperCase(); const map={DONE:["#1f6b3a","#e4f3ea"],CLOSE:["#4a5560","#eef1f0"],PENDING:["#8a5a00","#fdf1d8"],ONGOING:["#8a5a00","#fdf1d8"]}; const c=map[s]||["#4a5560","#eef1f0"]; return `<span class="pill" style="background:${c[1]};color:${c[0]};font-weight:700;">${esc(s||"—")}</span>`; }
function incOpen(i){ const s=(i.status||"").toUpperCase(); return s!=="DONE"&&s!=="CLOSE"; }
function incNteYes(i){ return (i.nte_issued||"").toUpperCase().startsWith("Y"); }
function incRepeatMap(){ const m={}; (INCIDENTS||[]).forEach(i=>{ const k=(i.person_name||"").trim().toLowerCase(); if(k) m[k]=(m[k]||0)+1; }); return m; }

function renderIncidents(){
  const pg=$("#page-incidents"); if(!pg) return;
  if(!canSeeIncidents()){ pg.innerHTML=`<div class="panel" style="margin-top:0;"><div class="psub">Incident Reports is restricted to the Director and HR (Manager, Relations, Payroll).</div></div>`; return; }
  const total=INCIDENTS.length;
  const nte=INCIDENTS.filter(incNteYes).length;
  const open=INCIDENTS.filter(incOpen).length;
  const rmap=incRepeatMap();
  const repeatPeople=Object.keys(rmap).filter(k=>rmap[k]>1).length;
  const sorted=INCIDENTS.slice().sort((a,b)=>{
    if(incOpen(a)!==incOpen(b)) return incOpen(a)?-1:1;
    return String(b.date_received||"").localeCompare(String(a.date_received||""));
  });
  pg.innerHTML=`
    <div class="panel" style="margin-top:0;">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;">
        <div><h2 style="margin:0;">Incident Reports</h2>
          <div class="psub">HO &amp; Disers incident log — from the store &amp; warehouse floor. Tracks the notice-to-explain, action taken, resolution and repeat offenders.</div></div>
      ...
      <input id="inSearch" class="search" style="width:100%;margin:10px 0 4px;" placeholder="Search person, branch, type, reporter, remarks…">
      ${total?`<table style="margin-top:6px;"><thead><tr><th>Incident</th><th>Person involved</th><th>Type</th><th>NTE</th><th>Status</th><th>Action</th></tr></thead>
        <tbody id="inRows"></tbody></table><div id="inCount" style="font-size:12px;color:var(--muted,#6b7785);margin-top:10px;"></div>`
      :`<div class="psub" style="margin-top:10px;">No incidents logged yet — click “＋ Log incident”.</div>`}
    </div>`;
  const bN=$("#inNew"); if(bN) bN.addEventListener("click",()=>openIncidentForm(null));
  let stFilter="All";
  const paint=()=>{
    const q=($("#inSearch")?$("#inSearch").value:"").trim().toLowerCase();
    const shown=sorted.filter(i=>stFilter==="All"||(i.status||"").toUpperCase()===stFilter)
      .filter(i=>!q || [i.incident_no,i.person_name,i.branch,i.incident_type,i.reporter_name,i.remarks,i.action_taken,i.office_type].filter(Boolean).join(" ").toLowerCase().includes(q));
    const rows=$("#inRows"); if(!rows) return;
    rows.innerHTML=shown.map(i=>{
      const rc=rmap[(i.person_name||"").trim().toLowerCase()]||0;
      const rep=rc>1?` <span class="pill" style="background:#fde8e8;color:#a12;font-weight:700;font-size:9.5px;">${rc}×</span>`:"";
      return `<tr class="clickable" data-iid="${esc(String(i.id))}">
        <td><b>${esc(i.incident_no||"—")}</b><div class="esub">${i.date_received?fmtDate(i.date_received):""}${i.branch?" · "+esc(i.branch):""}</div></td>
        <td>${esc(i.person_name||"—")}${rep}${i.person_position?`<div class="esub">${esc(i.person_position)}</div>`:""}</td>
        <td>${esc(i.incident_type||"—")}</td>
        <td>${incNteYes(i)?`<span class="pill" style="background:#e4f3ea;color:#1f6b3a;font-weight:700;">Yes</span>`:`<span class="pill" style="background:#eef1f0;color:#6b7785;">No</span>`}</td>
        <td>${incStatusPill(i.status)}</td>
        <td>${esc(i.action_taken||"—")}</td></tr>`; }).join("");
    const cc=$("#inCount"); if(cc) cc.textContent=`${shown.length} of ${total} incident${total!==1?"s":""}`;
    $$("#inRows tr.clickable").forEach(tr=>tr.addEventListener("click",()=>openIncident(INCIDENTS.find(x=>String(x.id)===tr.dataset.iid))));
  };
  $$("#inFilter .chip").forEach(c=>c.addEventListener("click",()=>{ $$("#inFilter .chip").forEach(x=>x.classList.remove("active")); c.classList.add("active"); stFilter=c.dataset.st; paint(); }));
  const s=$("#inSearch"); if(s) s.addEventListener("input",paint); paint();
}
...
function openIncident(i){
  ...
  m.style.cssText="position:fixed;inset:0;z-index:9998;background:rgba(14,50,25,.45);display:flex;justify-content:flex-end;";
  m.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:620px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    <div style="background:linear-gradient(135deg,#123524,#1f6b3a);color:#fff;padding:18px 22px;position:sticky;top:0;z-index:2;">
      <div style="font-size:20px;font-weight:800;">${esc(i.incident_no||"Incident")}</div>
      <div style="font-size:12.5px;opacity:.92;">${esc(i.incident_type||"—")}${i.branch?" · "+esc(i.branch):""}</div>
    </div>
    <div style="padding:16px 20px 70px;">
      <div class="panel" style="margin-top:0;">
...
function openIncidentForm(i){
  ...
  const opt=(arr,sel)=>arr.map(v=>`<option value="${esc(v)}"${v===sel?" selected":""}>${esc(v)}</option>`).join("");
  const empList=[...new Set((EMPLOYEES||[]).map(e=>e.full_name).filter(Boolean))].sort();
  let m=document.getElementById("inForm"); if(!m){ m=document.createElement("div"); m.id="inForm"; document.body.appendChild(m); }
  m.style.cssText="position:fixed;inset:0;z-index:10001;background:rgba(14,30,20,.55);display:flex;align-items:center;justify-content:center;padding:20px;";
  const fld=(label,inner)=>`<div style="margin-bottom:10px;"><div style="font-size:11.5px;font-weight:700;color:#4a5560;margin-bottom:3px;">${label}</div>${inner}</div>`;
  const inp=(id,val,ph,type)=>`<input id="${id}" ${type?`type="${type}"`:""} value="${val!=null?esc(String(val)):""}" placeholder="${ph||""}" style="width:100%;padding:8px 10px;border:1px solid #cdd6cf;border-radius:8px;font-size:13.5px;">`;
  const sel=(id,arr,val,blank)=>`<select id="${id}" style="width:100%;padding:8px 10px;border:1px solid #cdd6cf;border-radius:8px;font-size:13.5px;">${blank?`<option value="">—</option>`:""}${opt(arr,val)}</select>`;
  m.innerHTML=`<div style="background:#fff;border-radius:14px;max-width:660px;width:100%;max-height:92vh;overflow-y:auto;padding:22px;">
    <div style="font-size:10.5px;font-weight:800;letter-spacing:1.6px;color:#1f6b3a;">${isNew?"NEW":"EDIT"} INCIDENT REPORT</div>
    <div style="font-size:18px;font-weight:800;color:#123524;margin:2px 0 10px;">${isNew?"Log an incident":esc(i.incident_no||"Edit incident")}</div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      ${fld("Incident no.",inp("infNo",i.incident_no,isNew?"e.g. INC-0024":""))}
      ${fld("Date received by HR",inp("infReceived",i.date_received,"","date"))}
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      ${fld("Position",inp("infPersonPos",i.person_position,"e.g. MERCHANDISER"))}
    ...
    ${fld("Type of incident",`<input id="infType" list="infTypeList" value="${i.incident_type?esc(i.incident_type):""}" placeholder="Select or type…" style="width:100%;padding:8px 10px;border:1px solid #cdd6cf;border-radius:8px;font-size:13.5px;"><datalist id="infTypeList">${INCIDENT_TYPES.map(t=>`<option value="${esc(t)}"></option>`).join("")}</datalist>`)}
    <div style="border-top:1px solid #eef1f0;margin:6px 0 10px;"></div>
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;">
      ${fld("NTE issued",sel("infNte",["","YES","NO"],i.nte_issued!=null?String(i.nte_issued).toUpperCase():"",false))}
      ${fld("NTE date",inp("infNteDate",i.nte_date&&i.nte_date!=="-"?i.nte_date:"","","date"))}
      ${fld("Occurrence",sel("infCount",INCIDENT_COUNTS,i.incident_count,true))}
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      ${fld("Status",sel("infStatus",INCIDENT_STATUSES,i.status||"PENDING",false))}
      ${fld("Date resolved",inp("infResolved",i.date_resolved,"","date"))}
    </div>
    ${fld("Action taken",inp("infAction",i.action_taken,"e.g. FOR CHARGE, VOID, VERBAL WARNING, ADMIN HEARING"))}
    ${fld("Person in charge",inp("infIncharge",i.person_incharge,"e.g. JUVIE, GRAZEL"))}
    ${fld("Remarks",`<textarea id="infRemarks" rows="3" placeholder="Details, notes, resolution context…" style="width:100%;padding:8px 10px;border:1px solid #cdd6cf;border-radius:8px;font-size:13.5px;">${i.remarks?esc(i.remarks):""}</textarea>`)}
    ${fld("Upload attachments (IR, ATD, photos…)",`<input type="file" id="infFile" multiple style="font-size:12.5px;"><div id="infFileList" style="margin-top:6px;"></div>`)}
    <div id="infMsg" style="color:#a12;font-size:12.5px;min-height:16px;margin:2px 0;"></div>
    <div style="display:flex;gap:10px;margin-top:6px;">
      <button class="btn" id="infSave">${isNew?"Log incident":"Save changes"}</button>
      <button class="btn ghost" id="infCancel" style="margin-left:auto;">Cancel</button>
    </div>
  </div>`;
  m.addEventListener("click",ev=>{ if(ev.target===m) m.remove(); });
  ...
  const renderAttach=()=>{ const w=document.getElementById("infFileList"); if(!w) return;
    w.innerHTML = inAttach.length ? inAttach.map((a,ix)=>`<div style="display:flex;align-items:center;gap:8px;padding:4px 0;font-size:12.5px;"><span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">📎 ${esc(a.name)}</span><button type="button" class="btn ghost" data-axr="${ix}" style="padding:2px 8px;font-size:11.5px;flex-shrink:0;">Remove</button></div>`).join("") : `<div class="psub">No files attached yet.</div>`;
    w.querySelectorAll("[data-axr]").forEach(b=>b.onclick=()=>{ inAttach.splice(+b.dataset.axr,1); renderAttach(); });
  };
  renderAttach();
  const fInput=document.getElementById("infFile");
  if(fInput) fInput.onchange=async()=>{
    const files=[...fInput.files]; if(!files.length) return;
    const msg=document.getElementById("infMsg"); const setMsg=(t,c)=>{ if(msg){ msg.style.color=c||"#4a5560"; msg.textContent=t; } };
    for(const f of files){
      if(f.size>25*1024*1024){ setMsg(`“${f.name}” is over 25 MB — attach a smaller file.`, "#a12"); continue; }
      setMsg("Uploading "+f.name+"…");
      const path=`incidents/${key}/${Date.now()}-${f.name.replace(/[^\w.\-]+/g,"_")}`;
      const {error:upErr}=await sb.storage.from("loan-docs").upload(path,f,{upsert:true});
      if(upErr){ setMsg("Upload failed: "+upErr.message, "#a12"); continue; }
      inAttach.push({name:f.name, path, uploaded_at:new Date().toISOString(), uploaded_by:(CURRENT_USER&&CURRENT_USER.email)||null});
    }
    setMsg(""); fInput.value=""; renderAttach();
  };
  document.getElementById("infSave").onclick=()=>saveIncident(i,isNew,inAttach);
}
...
async function saveIncident(i,isNew,attachments){
  ...
  const payload={
    ...
    reporter_position:val("infReporterPos")||null,
    office_type:val("infOffice")||null,
    branch:val("infBranch")||null,
    person_name:person||null,
    person_position:val("infPersonPos")||null,
    emp_id: matched?(matched.employee_id||null):(i.emp_id||null),
    incident_type:val("infType")||null,
    nte_issued:val("infNte")||null,
    nte_date:val("infNteDate")||null,
    status:val("infStatus")||"PENDING",
    incident_count:val("infCount")||null,
    action_taken:val("infAction")||null,
    date_resolved:val("infResolved")||null,
    remarks:val("infRemarks")||null,
    person_incharge:val("infIncharge")||null,
    attachments:Array.isArray(attachments)?attachments:[],
    updated_at:new Date().toISOString(),
    updated_by:(CURRENT_USER&&CURRENT_USER.email)||null
  };
  const btn=document.getElementById("infSave"); if(btn){ btn.disabled=true; btn.textContent="Saving…"; }
  let error;
  if(isNew){ payload.created_by=(CURRENT_USER&&CURRENT_USER.email)||null; payload.created_at=new Date().toISOString(); ({ error }=await sb.from("incidents").insert(payload)); }
  else { ({ error }=await sb.from("incidents").update(payload).eq("id",i.id)); }
  if(error){ if(msg) msg.textContent="Could not save: "+error.message; if(btn){ btn.disabled=false; btn.textContent=isNew?"Log incident":"Save changes"; } return; }
  try{ await logChange("incident", isNew?null:i.id, payload.incident_no||person, isNew?"Incident logged":"Incident updated", payload.status); }catch(_){}
  const fm=document.getElementById("inForm"); if(fm) fm.remove();
  await loadIncidents(); renderIncidents(); updateNavBadges();
}
window.saveIncident=saveIncident;

async function deleteIncident(i){
  if(!i||!i.id) return;
  if(!confirm(`Delete incident “${i.incident_no||i.person_name||""}”? This cannot be undone.`)) return;
  const { error }=await sb.from("incidents").delete().eq("id",i.id);
  if(error){ alert("Could not delete: "+error.message); return; }
  try{ await logChange("incident", i.id, i.incident_no||i.person_name||"", "Incident deleted", ""); }catch(_){}
  const d=document.getElementById("inDrawer"); if(d) d.remove();
  await loadIncidents(); renderIncidents(); updateNavBadges();
}
window.deleteIncident=deleteIncident;

async function loadIncidents(){
  const { data }=await sb.from("incidents").select("*").order("date_received",{ascending:false});
  INCIDENTS=data||[];
}

/* ================= HMO COVERAGE — ICARE tier-based premium sharing =================
   Enrollee tracker + employer/employee premium sharing. The 5 tiers carry FIXED
   ER/EE amounts (ICARE 2026: ₱7,968.75 per head), so the money follows from the
   member's tier. Money page → admin + payroll (anj + Grazel). */
var HMO_MEMBERS = (typeof HMO_MEMBERS!=="undefined" && HMO_MEMBERS) || [];
const HMO_PREMIUM=7968.75, HMO_GROUP_LIFE=10000, HMO_MBL=70000;
const HMO_TIERS={
  cat1:{label:"Category 1 · General Staff (1–8 yrs)", short:"Cat 1", ratio:"60/40", er:4781.25, ee:3187.50, freq:"4 deductions", perCut:796.88},
  cat2:{label:"Category 2 · General Staff (8+ yrs)",  short:"Cat 2", ratio:"70/30", er:5578.13, ee:2390.63, freq:"3 deductions", perCut:796.88},
  cat3:{label:"Category 3 · Supervisory",             short:"Cat 3", ratio:"80/20", er:6375.00, ee:1593.75, freq:"2 deductions", perCut:796.88},
  cat4:{label:"Category 4 · Top Management",          short:"Cat 4", ratio:"100/0", er:7968.75, ee:0.00,    freq:"—",           perCut:0},
  lead:{label:"Lead Diser",                           short:"Lead",  ratio:"50/50", er:3984.38, ee:3984.38, freq:"4 deductions", perCut:996.09}
};
const HMO_TIER_ORDER=["cat1","cat2","cat3","cat4","lead"];
function canSeeHmo(){ return canSeePay(); }
function hmoEmp(m){ if(!m||!m.emp_id) return null; return (EMPLOYEES||[]).find(e=>String(e.employee_id||"")===String(m.emp_id)); }
function hmoName(m){ const e=hmoEmp(m); if(e&&e.full_name) return e.full_name; return [m.first_name,m.last_name].filter(Boolean).join(" ")||"—"; }
function hmoPosition(m){ const e=hmoEmp(m); return (e&&e.position)||m.position||"—"; }
function hmoHire(m){ const e=hmoEmp(m); return (e&&(e.hire_date||e.date_hired))||m.date_hired||null; }
function hmoTenureYears(m){ const h=hmoHire(m); if(!h) return null; const d=new Date(h); if(isNaN(d)) return null; return (Date.now()-d.getTime())/(365.25*24*3600*1000); }
function hmoTier(m){ return HMO_TIERS[m&&m.tier]||null; }
function hmoIsDep(m){ return (m&&m.member_type||"P").toUpperCase().startsWith("D"); }
function hmoDeps(m){ if(!m) return []; return (HMO_MEMBERS||[]).filter(x=>hmoIsDep(x) && String(x.principal_emp_id||"")===String(m.emp_id||"~none")); }
function hmoTierPill(t){ const T=HMO_TIERS[t]; if(!T) return `<span class="pill" style="background:#eef1f0;color:#6b7785;">—</span>`; const col=t==="cat4"?["#123524","#dbeadf"]:t==="cat3"?["#1f6b3a","#e4f3ea"]:t==="lead"?["#8a5a00","#fdf1d8"]:["#31506a","#e6eef6"]; return `<span class="pill" style="background:${col[1]};color:${col[0]};font-weight:700;">${esc(T.short)} · ${esc(T.ratio)}</span>`; }

function renderHmo(){
  const pg=$("#page-hmo"); if(!pg) return;
  if(!canSeeHmo()){ pg.innerHTML=`<div class="panel" style="margin-top:0;"><div class="psub">HMO Coverage is restricted to the Director and Payroll (premium &amp; payroll-deduction data).</div></div>`; return; }
  const principals=HMO_MEMBERS.filter(m=>!hmoIsDep(m));
  const active=principals.filter(m=>(m.status||"Active")==="Active");
  const erTot=active.reduce((s,m)=>s+((hmoTier(m)||{}).er||0),0);
  const eeTot=active.reduce((s,m)=>s+((hmoTier(m)||{}).ee||0),0);
  const annual=active.length*HMO_PREMIUM;
  const crossings=active.filter(m=>m.tier==="cat1"&&(hmoTenureYears(m)||0)>=7.5);
  const depCount=HMO_MEMBERS.filter(hmoIsDep).length;
  const tierCounts={}; HMO_TIER_ORDER.forEach(t=>tierCounts[t]=active.filter(m=>m.tier===t).length);
  const sorted=principals.slice().sort((a,b)=>{ const ta=HMO_TIER_ORDER.indexOf(a.tier), tb=HMO_TIER_ORDER.indexOf(b.tier); if(ta!==tb) return ta-tb; return hmoName(a).localeCompare(hmoName(b)); });
  pg.innerHTML=`
    <div class="panel" style="margin-top:0;">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;">
        <div><h2 style="margin:0;">HMO Coverage — ICARE ${esc(String((HMO_MEMBERS[0]&&HMO_MEMBERS[0].policy_year)||2026))}</h2>
          <div class="psub">Tier-based employer / employee premium sharing. Tier is driven by <b>tenure &amp; position</b>; the ER/EE amounts follow the tier automatically. MBL ₱${HMO_MBL.toLocaleString()} · Ward · ₱${HMO_GROUP_LIFE.toLocaleString()} group life bundled.</div></div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <button class="btn" id="hmNew">＋ Enrol member</button>
          <button class="btn ghost" id="hmNewDep">＋ Add dependent</button>
        </div>
      </div>
    ...
    <div class="panel">
      <h2>Tier structure — ICARE ${esc(String((HMO_MEMBERS[0]&&HMO_MEMBERS[0].policy_year)||2026))}</h2>
      <div class="psub">Premium per head ₱${HMO_PREMIUM.toLocaleString()} · ER + EE share is fixed per tier.</div>
      <table><thead><tr><th>Tier</th><th>Eligibility</th><th>Sharing</th><th>ER ₱</th><th>EE ₱</th><th>Per cut-off</th><th>Members</th></tr></thead><tbody>
        ${HMO_TIER_ORDER.map(t=>{ const T=HMO_TIERS[t]; return `<tr><td><b>${esc(T.short)}</b></td><td>${esc(T.label.replace(/^Category \d+ · /,""))}</td><td>${esc(T.ratio)}</td><td>${peso(T.er)}</td><td>${peso(T.ee)}</td><td>${T.perCut?peso(T.perCut)+" × "+esc(T.freq.replace(/[^0-9]/g,"")||""):"—"}</td><td>${tierCounts[t]}</td></tr>`; }).join("")}
      </tbody></table>
    </div>

    <div class="panel">
      <h2>Members</h2>
      <div class="filterbar" id="hmFilter">
        <div class="chip active" data-t="All">All (${principals.length})</div>
        ${HMO_TIER_ORDER.map(t=>`<div class="chip" data-t="${t}">${esc(HMO_TIERS[t].short)} · ${esc(HMO_TIERS[t].ratio)} (${tierCounts[t]})</div>`).join("")}
      </div>
      <input id="hmSearch" class="search" style="width:100%;margin:10px 0 4px;" placeholder="Search name, emp ID, position…">
      <table style="margin-top:6px;"><thead><tr><th>Emp ID</th><th>Name</th><th>Position</th><th>Tenure</th><th>Tier</th><th>ER / EE</th><th>Deps</th></tr></thead>
        <tbody id="hmRows"></tbody></table>
      <div id="hmCount" style="font-size:12px;color:var(--muted,#6b7785);margin-top:10px;"></div>
    </div>`;
  const bN=$("#hmNew"); if(bN) bN.addEventListener("click",()=>openHmoForm(null,false));
  const bD=$("#hmNewDep"); if(bD) bD.addEventListener("click",()=>openHmoForm(null,true));
  let tFilter="All";
  const paint=()=>{
    const q=($("#hmSearch")?$("#hmSearch").value:"").trim().toLowerCase();
    const shown=sorted.filter(m=>tFilter==="All"||m.tier===tFilter)
      .filter(m=>!q || [hmoName(m),m.emp_id,hmoPosition(m)].filter(Boolean).join(" ").toLowerCase().includes(q));
    const rows=$("#hmRows"); if(!rows) return;
    rows.innerHTML=shown.map(m=>{ const T=hmoTier(m); const ty=hmoTenureYears(m); const cross=m.tier==="cat1"&&(ty||0)>=7.5; const nd=hmoDeps(m).length;
      return `<tr class="clickable" data-hid="${esc(String(m.id))}"${cross?' style="background:#fff9ea;"':''}>
        <td>${esc(m.emp_id||"—")}</td>
        <td><b>${esc(hmoName(m))}</b>${m.remarks?`<div class="esub">${esc(m.remarks)}</div>`:""}</td>
        <td>${esc(hmoPosition(m))}</td>
        <td>${ty!=null?ty.toFixed(1)+"y":"—"}${cross?` <span class="pill" style="background:#fde8c8;color:#8a5a00;font-weight:700;font-size:9.5px;">→ Cat 2</span>`:""}</td>
        <td>${hmoTierPill(m.tier)}</td>
        <td>${T?peso(T.er)+" / "+peso(T.ee):"—"}</td>
        <td>${nd||"—"}</td></tr>`; }).join("");
    const cc=$("#hmCount"); if(cc) cc.textContent=`${shown.length} of ${principals.length} member${principals.length!==1?"s":""}`;
    $$("#hmRows tr.clickable").forEach(tr=>tr.addEventListener("click",()=>openHmoMember(HMO_MEMBERS.find(x=>String(x.id)===tr.dataset.hid))));
  };
  $$("#hmFilter .chip").forEach(c=>c.addEventListener("click",()=>{ $$("#hmFilter .chip").forEach(x=>x.classList.remove("active")); c.classList.add("active"); tFilter=c.dataset.t; paint(); }));
  const s=$("#hmSearch"); if(s) s.addEventListener("input",paint); paint();
}
window.renderHmo=renderHmo;

function openHmoMember(m){
  if(!m) return;
  const T=hmoTier(m); const ty=hmoTenureYears(m); const deps=hmoDeps(m);
  const kv=(k,val)=>`<div class="efield"><div class="el">${esc(k)}</div><div class="ev">${val}</div></div>`;
  let d=document.getElementById("hmDrawer"); if(!d){ d=document.createElement("div"); d.id="hmDrawer"; document.body.appendChild(d); }
  d.style.cssText="position:fixed;inset:0;z-index:9998;background:rgba(14,50,25,.45);display:flex;justify-content:flex-end;";
  d.innerHTML=`<div style="background:#f1f4f2;width:100%;max-width:620px;height:100%;overflow-y:auto;box-shadow:-6px 0 30px rgba(0,0,0,.18);">
    <div style="background:linear-gradient(135deg,#123524,#1f6b3a);color:#fff;padding:18px 22px;position:sticky;top:0;z-index:2;">
      <div style="font-size:20px;font-weight:800;">${esc(hmoName(m))}</div>
      <div style="font-size:12.5px;opacity:.92;">${esc(hmoPosition(m))}${m.emp_id?" · "+esc(m.emp_id):""}${hmoIsDep(m)?" · DEPENDENT":""}</div>
    </div>
    <div style="padding:16px 20px 70px;">
      <div class="panel" style="margin-top:0;">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;"><h2 style="margin:0;">Coverage</h2>${hmoTierPill(m.tier)}</div>
        <div class="egrid" style="margin-top:8px;">
          ${kv("Provider / year",esc((m.hmo_provider||"ICARE")+" "+(m.policy_year||2026)))}
          ${kv("Benefit / MBL",esc((m.benefit_class||"WARD"))+" · ₱"+Number(m.mbl||HMO_MBL).toLocaleString())}
          ${kv("Tenure",ty!=null?ty.toFixed(1)+" years":"—")}
          ${kv("Sharing",T?T.ratio:"—")}
          ${kv("Employer share",T?peso(T.er):"—")}
          ${kv("Employee share",T?peso(T.ee):"—")}
          ${kv("Payroll deduction",T&&T.perCut?peso(T.perCut)+" × "+esc(T.freq):"—")}
          ${kv("Effective",m.effective_date?fmtDate(m.effective_date):"—")}
          ${kv("Expiry",m.expiry_date?fmtDate(m.expiry_date):"—")}
          ${kv("Status",esc(m.status||"Active"))}
          ${m.vip_tagging?kv("VIP tag",esc(m.vip_tagging)):""}
        </div>
        ${m.remarks?`<div style="margin-top:8px;font-size:13px;color:#22302a;"><b>Remarks:</b> ${esc(m.remarks)}</div>`:""}
      </div>
      ${!hmoIsDep(m)?`<div class="panel"><div style="display:flex;align-items:center;justify-content:space-between;"><div class="subhead">Dependents <span class="count-tag">${deps.length}</span></div><button class="btn ghost" id="hmAddDep" style="padding:4px 10px;font-size:12px;">＋ Add</button></div>
        ${deps.length?deps.map(dp=>`<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-top:1px solid #e4eae6;font-size:13px;"><span style="flex:1;">${esc(hmoName(dp))}${dp.relationship?` · ${esc(dp.relationship)}`:""}</span><button class="btn ghost" data-editdep="${esc(String(dp.id))}" style="padding:2px 8px;font-size:11.5px;">Edit</button></div>`).join(""):`<div class="psub" style="margin-top:6px;">No dependents on file.</div>`}</div>`:(m.principal_emp_id?`<div class="panel"><div class="subhead">Principal</div><div style="font-size:13px;">${esc((()=>{ const p=HMO_MEMBERS.find(x=>String(x.emp_id||"")===String(m.principal_emp_id)&&!hmoIsDep(x)); return p?hmoName(p)+" · "+(m.principal_emp_id):m.principal_emp_id; })())}</div></div>`:"")}
      <div class="psub" style="margin-top:2px;">${m.updated_at?"Updated "+fmtDate(m.updated_at):(m.created_at?"Created "+fmtDate(m.created_at):"")}</div>
      <div style="display:flex;gap:10px;margin-top:10px;flex-wrap:wrap;">
        <button class="btn" id="hmEdit">Edit</button>
        <button class="btn ghost" id="hmDel" style="color:#a12;border-color:#e9b9b9;">Delete</button>
        <button class="btn ghost" id="hmCloseBtn" style="margin-left:auto;">Close</button>
      </div>
    </div></div>`;
  d.addEventListener("click",ev=>{ if(ev.target===d) d.remove(); });
  document.getElementById("hmCloseBtn").onclick=()=>d.remove();
  document.getElementById("hmEdit").onclick=()=>{ d.remove(); openHmoForm(m,hmoIsDep(m)); };
  document.getElementById("hmDel").onclick=()=>deleteHmoMember(m);
  const ad=document.getElementById("hmAddDep"); if(ad) ad.onclick=()=>{ d.remove(); openHmoForm({principal_emp_id:m.emp_id, tier:m.tier},true); };
  d.querySelectorAll("[data-editdep]").forEach(b=>b.onclick=()=>{ d.remove(); openHmoForm(HMO_MEMBERS.find(x=>String(x.id)===b.dataset.editdep),true); });
}
window.openHmoMember=openHmoMember;

function openHmoForm(m,isDep){
  const isNew=!m||!m.id; m=m||{};
  const opt=(arr,sel)=>arr.map(v=>`<option value="${esc(v.v!=null?v.v:v)}"${(v.v!=null?v.v:v)===sel?" selected":""}>${esc(v.l!=null?v.l:v)}</option>`).join("");
  const empList=[...new Set((EMPLOYEES||[]).map(e=>e.full_name).filter(Boolean))].sort();
  const principals=HMO_MEMBERS.filter(x=>!hmoIsDep(x));
  let mo=document.getElementById("hmForm"); if(!mo){ mo=document.createElement("div"); mo.id="hmForm"; document.body.appendChild(mo); }
  mo.style.cssText="position:fixed;inset:0;z-index:10001;background:rgba(14,30,20,.55);display:flex;align-items:center;justify-content:center;padding:20px;";
  const fld=(label,inner)=>`<div style="margin-bottom:10px;"><div style="font-size:11.5px;font-weight:700;color:#4a5560;margin-bottom:3px;">${label}</div>${inner}</div>`;
  const inp=(id,val,ph,type)=>`<input id="${id}" ${type?`type="${type}"`:""} value="${val!=null?esc(String(val)):""}" placeholder="${ph||""}" style="width:100%;padding:8px 10px;border:1px solid #cdd6cf;border-radius:8px;font-size:13.5px;">`;
  const tierOpts=HMO_TIER_ORDER.map(t=>({v:t,l:HMO_TIERS[t].short+" · "+HMO_TIERS[t].ratio}));
  mo.innerHTML=`<div style="background:#fff;border-radius:14px;max-width:620px;width:100%;max-height:92vh;overflow-y:auto;padding:22px;">
    <div style="font-size:10.5px;font-weight:800;letter-spacing:1.6px;color:#1f6b3a;">${isNew?"NEW":"EDIT"} HMO ${isDep?"DEPENDENT":"MEMBER"}</div>
    <div style="font-size:18px;font-weight:800;color:#123524;margin:2px 0 10px;">${isNew?(isDep?"Add a dependent":"Enrol a member"):esc(hmoName(m))}</div>
    ${isDep?`
      ${fld("Principal (employee) *",`<select id="hmfPrincipal" style="width:100%;padding:8px 10px;border:1px solid #cdd6cf;border-radius:8px;font-size:13.5px;"><option value="">— select principal —</option>${principals.map(p=>`<option value="${esc(p.emp_id||"")}"${String(p.emp_id||"")===String(m.principal_emp_id||"")?" selected":""}>${esc(hmoName(p))}${p.emp_id?" · "+esc(p.emp_id):""}</option>`).join("")}</select>`)}
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        ${fld("First name",inp("hmfFirst",m.first_name,"dependent first name"))}
        ${fld("Last name",inp("hmfLast",m.last_name,"dependent last name"))}
      </div>
      ${fld("Relationship",`<select id="hmfRel" style="width:100%;padding:8px 10px;border:1px solid #cdd6cf;border-radius:8px;font-size:13.5px;">${opt(["","Spouse","Child","Parent","Sibling","Domestic partner","Other"],m.relationship||"")}</select>`)}
    `:`
      ${fld("Employee *",`<input id="hmfEmp" list="hmfEmpList" value="${m.first_name||m.last_name?esc(hmoName(m)):""}" placeholder="Search employee…" style="width:100%;padding:8px 10px;border:1px solid #cdd6cf;border-radius:8px;font-size:13.5px;"><datalist id="hmfEmpList">${empList.map(n=>`<option value="${esc(n)}"></option>`).join("")}</datalist>`)}
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        ${fld("Employee ID",inp("hmfEmpId",m.emp_id,"PayPlus ID"))}
        ${fld("Member no.",inp("hmfNo",m.member_no,"","number"))}
      </div>
      ${fld("Tier *",`<select id="hmfTier" style="width:100%;padding:8px 10px;border:1px solid #cdd6cf;border-radius:8px;font-size:13.5px;"><option value="">— select tier —</option>${opt(tierOpts,m.tier||"")}</select>`)}
      <div id="hmfTierHint" class="psub" style="margin:-4px 0 8px;"></div>
    `}
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      ${fld("Effective date",inp("hmfEff",m.effective_date||"2026-01-27","","date"))}
      ${fld("Expiry date",inp("hmfExp",m.expiry_date||"2027-01-27","","date"))}
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      ${fld("Status",`<select id="hmfStatus" style="width:100%;padding:8px 10px;border:1px solid #cdd6cf;border-radius:8px;font-size:13.5px;">${opt(["Active","Suspended","For reversal","Separated"],m.status||"Active")}</select>`)}
      ${fld("VIP tag",inp("hmfVip",m.vip_tagging,"optional"))}
    </div>
    ${fld("Remarks",`<textarea id="hmfRemarks" rows="2" placeholder="e.g. New Enrollee, For Reversal Final Pay…" style="width:100%;padding:8px 10px;border:1px solid #cdd6cf;border-radius:8px;font-size:13.5px;">${m.remarks?esc(m.remarks):""}</textarea>`)}
    <div id="hmfMsg" style="color:#a12;font-size:12.5px;min-height:16px;margin:2px 0;"></div>
    <div style="display:flex;gap:10px;margin-top:6px;">
      <button class="btn" id="hmfSave">${isNew?(isDep?"Add dependent":"Enrol"):"Save changes"}</button>
      <button class="btn ghost" id="hmfCancel" style="margin-left:auto;">Cancel</button>
    </div>
  </div>`;
  mo.addEventListener("click",ev=>{ if(ev.target===mo) mo.remove(); });
  document.getElementById("hmfCancel").onclick=()=>mo.remove();
  if(!isDep){
    const empIn=document.getElementById("hmfEmp"), idIn=document.getElementById("hmfEmpId");
    if(empIn) empIn.addEventListener("change",()=>{ const e=(EMPLOYEES||[]).find(x=>x.full_name===empIn.value); if(e&&idIn&&!idIn.value) idIn.value=e.employee_id||""; });
    const tierSel=document.getElementById("hmfTier"), hint=document.getElementById("hmfTierHint");
    const showHint=()=>{ const T=HMO_TIERS[tierSel.value]; if(hint) hint.textContent=T?`ER ${peso(T.er)} · EE ${peso(T.ee)} · ${T.perCut?peso(T.perCut)+" per cut-off ("+T.freq+")":"no employee share"}`:""; };
    if(tierSel){ tierSel.addEventListener("change",showHint); showHint(); }
  }
  document.getElementById("hmfSave").onclick=()=>saveHmoMember(m,isNew,isDep);
}
window.openHmoForm=openHmoForm;

async function saveHmoMember(m,isNew,isDep){
  const val=id=>{ const el=document.getElementById(id); return el?el.value.trim():""; };
  const msg=document.getElementById("hmfMsg");
  let payload={
    effective_date:val("hmfEff")||null,
    expiry_date:val("hmfExp")||null,
    status:val("hmfStatus")||"Active",
    vip_tagging:val("hmfVip")||null,
    remarks:val("hmfRemarks")||null,
    updated_at:new Date().toISOString(),
    updated_by:(CURRENT_USER&&CURRENT_USER.email)||null
  };
  if(isDep){
    const principal=val("hmfPrincipal");
    if(!principal){ if(msg) msg.textContent="Select the principal employee."; return; }
    const P=HMO_MEMBERS.find(x=>String(x.emp_id||"")===String(principal)&&!hmoIsDep(x));
    payload.member_type="D"; payload.principal_emp_id=principal;
    payload.first_name=val("hmfFirst")||null; payload.last_name=val("hmfLast")||null;
    payload.relationship=val("hmfRel")||null;
    payload.tier=P?P.tier:(m.tier||null);
  } else {
    const empName=val("hmfEmp"), tier=val("hmfTier");
    if(!empName){ if(msg) msg.textContent="Select the employee."; return; }
    if(!tier){ if(msg) msg.textContent="Select the coverage tier."; return; }
    const e=(EMPLOYEES||[]).find(x=>x.full_name===empName);
    payload.member_type="P";
    payload.emp_id=val("hmfEmpId")||(e&&e.employee_id)||null;
    payload.member_no=val("hmfNo")?Number(val("hmfNo")):(m.member_no||null);
    payload.tier=tier; payload.tier_override=(m.tier&&m.tier!==tier)?true:(m.tier_override||false);
    if(e){ const parts=(e.full_name||"").split(" "); } // name comes from the linked employee at render time
    if(!payload.emp_id && e===undefined){ payload.first_name=empName; }
  }
  const btn=document.getElementById("hmfSave"); if(btn){ btn.disabled=true; btn.textContent="Saving…"; }
  let error;
  if(isNew){ payload.created_by=(CURRENT_USER&&CURRENT_USER.email)||null; payload.created_at=new Date().toISOString(); payload.hmo_provider=payload.hmo_provider||"ICARE"; payload.policy_year=payload.policy_year||2026; ({ error }=await sb.from("hmo_members").insert(payload)); }
  else { ({ error }=await sb.from("hmo_members").update(payload).eq("id",m.id)); }
  if(error){ if(msg) msg.textContent="Could not save: "+error.message; if(btn){ btn.disabled=false; btn.textContent=isNew?"Enrol":"Save changes"; } return; }
  try{ await logChange("hmo", isNew?null:m.id, payload.first_name||payload.emp_id||"", isNew?"HMO enrolled":"HMO updated", payload.status); }catch(_){}
  const fm=document.getElementById("hmForm"); if(fm) fm.remove();
  await loadHmo(); renderHmo();
}
window.saveHmoMember=saveHmoMember;

async function deleteHmoMember(m){
  if(!m||!m.id) return;
  const deps=hmoIsDep(m)?[]:hmoDeps(m);
  if(deps.length){ alert(`Remove the ${deps.length} dependent(s) first.`); return; }
  if(!confirm(`Remove ${hmoName(m)} from HMO coverage? This cannot be undone.`)) return;
  const { error }=await sb.from("hmo_members").delete().eq("id",m.id);
  if(error){ alert("Could not delete: "+error.message); return; }
  try{ await logChange("hmo", m.id, hmoName(m), "HMO removed", ""); }catch(_){}
  const d=document.getElementById("hmDrawer"); if(d) d.remove();
  await loadHmo(); renderHmo();
}
window.deleteHmoMember=deleteHmoMember;
...
/* ===================================================================
   DOCUMENTS TO SIGN — scoped co-signer page (Margie / Accounting)
   A non-admin "cosigner" logs in and sees ONLY the documents awaiting
   HER signature — the loan co-sign rows routed to her in external_signoffs.
   She opens a document, reviews the agreement, then uploads or draws her
   signature (captureSignatureModal already handles upload + draw + knockout).
   Additive: reuses EXT_SIGNOFFS + external_signoffs, no new tables.
   =================================================================== */
// Inject the nav item + page container from JS so app.js stays the only file touched.
// Idempotent — safe to call repeatedly; runs BEFORE applyRoleUI so role gating sees the item.
function ensureCosignDom(){
  try{ if(window.TITLES && !window.TITLES.cosign) window.TITLES.cosign="Documents to Sign"; }catch(_){}
  if(!document.getElementById("page-cosign")){
    const anchor=document.getElementById("page-links")||document.querySelector("section.page");
    if(anchor && anchor.parentNode){
      const sec=document.createElement("section"); sec.className="page"; sec.id="page-cosign";
      anchor.parentNode.insertBefore(sec, anchor.nextSibling);
    }
  }
  if(!document.querySelector('.nav-item[data-page="cosign"]')){
    const ref=document.querySelector('.nav-item[data-page="links"]')||document.querySelector('.nav-item[data-page]');
    if(ref && ref.parentNode){
      const nav=document.createElement("div"); nav.className="nav-item"; nav.setAttribute("data-page","cosign");
      nav.setAttribute("onclick","go('cosign')");
      nav.innerHTML=`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>Documents to Sign`;
...
function renderCosign(){
  ...
  const relCard=(l)=>`
    <div class="lk-row" style="display:flex;align-items:center;gap:12px;border-top:1px solid var(--line,#e4eae6);padding:11px 0;">
      <div style="flex:1;min-width:0;">
        <div style="font-weight:700;color:#12352a;">${esc(l.applicant_name||"")}${(typeof loanRushBadgeHtml==="function")?loanRushBadgeHtml(l):""}</div>
        <div class="psub" style="margin:2px 0 0;">${esc(l.loan_ref||"")} · ${peso(l.amount)}</div>
        <div class="psub" style="margin:2px 0 0;">Release: <b>${l.release_date?fmtDate(l.release_date):"—"}</b> · 1st deduction: ${l.first_deduction_date?fmtDate(l.first_deduction_date):"—"}</div>
      </div>
...
function openCosignDoc(id){
  const row=(EXT_SIGNOFFS||[]).find(s=>String(s.id)===String(id)); if(!row) return;
  let m=document.getElementById("cosignDocModal"); if(!m){ m=document.createElement("div"); m.id="cosignDocModal"; document.body.appendChild(m); }
  m.style.cssText="position:fixed;inset:0;z-index:10001;background:rgba(14,30,50,.55);display:flex;align-items:center;justify-content:center;padding:20px;";
  m.innerHTML=`<div style="background:#fff;border-radius:14px;max-width:720px;width:100%;max-height:92vh;display:flex;flex-direction:column;overflow:hidden;">
    <div style="padding:18px 22px 8px;">
      <div style="font-size:18px;font-weight:800;color:#12352a;">Review &amp; sign</div>
      <div class="psub" style="margin:2px 0 0;">${esc(row.context_label||"Document")}${row.signer_role?" · "+esc(row.signer_role):""}</div>
    </div>
    <div style="flex:1;overflow-y:auto;padding:0 22px 4px;">
      <div style="border:1px solid var(--line,#e4eae6);border-radius:10px;padding:14px;background:#fff;">${row.document_html||'<div class="psub">No document content on file.</div>'}</div>
    </div>
    <div style="display:flex;gap:10px;padding:14px 22px;border-top:1px solid var(--line,#e4eae6);">
      <button class="btn ghost" id="cosignClose" type="button" style="margin-left:auto;">Close</button>
      <button class="btn" id="cosignSign" type="button">Add your signature</button>
    </div></div>`;
  m.addEventListener("click",ev=>{ if(ev.target===m) m.remove(); });
  document.getElementById("cosignClose").onclick=()=>m.remove();
  document.getElementById("cosignSign").onclick=()=>{
    captureSignatureModal({ title:"Sign this document", subtitle:row.context_label||"", cta:"Sign & submit",
      onSign:(data)=>{ if(!data) return;
        sb.from("external_signoffs").update({ status:"Signed", signature_data:data, signed_name:(myName()||row.signer_name||null), signed_at:new Date().toISOString() }).eq("id", row.id).then(async ({error})=>{
          if(error){ alert(error.message); return; }
          await loadEmployees();
          const dm=document.getElementById("cosignDocModal"); if(dm) dm.remove();
          renderCosign();
        });
      }
    });
...
window.openCosignDoc=openCosignDoc;
...
async function deskIdeaRemove(id){ if(!confirm("Remove this idea permanently? This deletes it for good.\n\n(Use Archive instead if you just want it out of the active list but kept on record.)")) return; await sb.from("hr_ideas").delete().eq("id",id); await reloadDesk(); }
/* tasks */
...
function renderDesk(){
  ...
  const ideaInbox=admin?`<div style="margin-top:12px;"><div class="subhead">Ideas inbox <span class="count-tag">${HR_IDEAS.length}</span></div>
    ${HR_IDEAS.length?HR_IDEAS.map(i=>`<div class="task" style="align-items:flex-start;flex-wrap:wrap;"><div class="dot ${i.status==='New'?'a':'g'}"></div><div style="flex:1;min-width:200px;"><div class="tt">${esc(i.title)} <span style="font-size:11px;color:#8a938c;">· ${esc(i.status)}</span></div><div class="td">from ${esc(i.from_name||i.from_email)}${i.detail?(' · '+esc(i.detail)):''}</div>
      ${i.admin_comment?`<div style="margin-top:5px;background:#eef4ef;border:1px solid #cfe0d4;border-radius:8px;padding:6px 9px;font-size:12px;"><b>anj:</b> ${esc(i.admin_comment)}${i.admin_comment_at?` <span style="color:#8a938c;">· ${fmtDate(i.admin_comment_at)}</span>`:''}</div>`:''}
      <div style="display:flex;gap:6px;margin-top:6px;">
        <input id="ideacmt_${i.id}" placeholder="${i.admin_comment?'Edit your reply…':'Reply / comment to the sender…'}" value="${esc(i.admin_comment||'')}" style="flex:1;padding:6px 9px;border:1px solid var(--line);border-radius:7px;font-size:12.5px;">
        <button ${DMINI} data-ideacomment="${i.id}">Save reply</button>
      </div></div>
      <div style="display:flex;gap:6px;">${i.status!=='Reviewed'?`<button ${DMINI} data-ideastatus="Reviewed" data-idea="${i.id}">Reviewed</button>`:''}${i.status!=='Actioned'?`<button ${DMINI} data-ideastatus="Actioned" data-idea="${i.id}">Actioned</button>`:''}${i.status!=='Archived'?`<button ${DMINI} data-ideastatus="Archived" data-idea="${i.id}">Archive</button>`:''}<button ${DMINI} data-idearemove="${i.id}" style="color:#a12;">Remove</button></div></div>`).join(""):`<div class="psub">No ideas submitted yet.</div>`}</div>`
  ...
  q('[data-idearemove]').forEach(b=>b.onclick=()=>deskIdeaRemove(+b.dataset.idearemove));
...
async function renderActivity(){
  ...
  // Weekly backups (owners only) — read the log; RLS restricts it to owners.
  let backups=[]; try{ backups=(await sb.from("backup_log").select("*").order("created_at",{ascending:false})).data||[]; }catch(_){}
  const mb=n=>(Number(n||0)/1048576).toFixed(2)+" MB";
  const backupRows = backups.length ? backups.map(b=>`<tr>
      <td style="padding:7px 9px;border-bottom:0.5px solid var(--line);"><b>${fmtDateTime(b.created_at)}</b></td>
      <td style="padding:7px 9px;border-bottom:0.5px solid var(--line);">${b.trigger==='manual'?'Manual':'Friday auto'}${b.created_by?` <span class="esub">${esc(b.created_by)}</span>`:''}</td>
      <td style="padding:7px 9px;border-bottom:0.5px solid var(--line);">${b.table_count||0} tables · ${(b.row_count||0).toLocaleString()} rows</td>
      <td style="padding:7px 9px;border-bottom:0.5px solid var(--line);">${mb(b.size_bytes)}</td>
      <td style="padding:7px 9px;border-bottom:0.5px solid var(--line);text-align:right;"><button class="btn ghost" onclick="downloadBackup('${esc(b.filename)}')">⬇ Download</button></td>
    </tr>`).join("") : `<tr><td colspan="5" class="psub" style="padding:9px;">No backups yet — click <b>Back up now</b> to create the first one.</td></tr>`;
  pg.innerHTML=`
    <div class="panel" style="margin-top:0;">
    ...
    <div class="panel">
      <h2>Weekly Backups <span class="count-tag">owners only</span></h2>
      <div class="psub">A full snapshot of the entire HRIS database is saved automatically <b>every Friday, 6:00&nbsp;PM</b>. Download any week's file to keep an off-site copy (e.g. in your Dropbox) — that downloaded file is your safety net if anything is ever lost. The latest 12 are kept.</div>
      <div style="margin:11px 0;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
        <button class="btn" id="backupNowBtn" onclick="backupNow()">Back up now</button>
        <span id="backupMsg" class="psub"></span>
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:13px;"><thead><tr style="text-align:left;color:var(--muted);">
        <th style="padding:6px 9px;font-weight:600;">When</th><th style="padding:6px 9px;font-weight:600;">Type</th><th style="padding:6px 9px;font-weight:600;">Contents</th><th style="padding:6px 9px;font-weight:600;">Size</th><th style="padding:6px 9px;font-weight:600;text-align:right;">File</th></tr></thead>
...
// ---- Weekly database backups (owner-only) ----
// "Back up now" → the db-backup edge function snapshots every table to the private bucket.
async function backupNow(){
  const btn=document.getElementById("backupNowBtn"), msg=document.getElementById("backupMsg");
  if(btn){ btn.disabled=true; btn.textContent="Backing up…"; } if(msg){ msg.textContent="Snapshotting the whole database…"; msg.style.color="var(--muted)"; }
  try{
    const { data, error } = await sb.functions.invoke("db-backup",{ body:{ trigger:"manual" } });
    if(error||!data||!data.ok){ if(msg){ msg.textContent="Backup failed: "+((data&&data.error)||(error&&error.message)||"unknown"); msg.style.color="#a4322a"; } }
    else if(msg){ msg.textContent="✓ Backed up "+data.table_count+" tables · "+(Number(data.row_count||0)).toLocaleString()+" rows · "+(Number(data.size_bytes||0)/1048576).toFixed(2)+" MB."; msg.style.color="var(--green-dark)"; }
  }catch(e){ if(msg){ msg.textContent="Backup failed: "+e.message; msg.style.color="#a4322a"; } }
  if(btn){ btn.disabled=false; btn.textContent="Back up now"; }
  try{ await renderActivity(); }catch(_){}
}
window.backupNow=backupNow;
// Download a backup file — a short-lived signed URL (owners only, enforced by storage RLS).
async function downloadBackup(filename){
  try{
    const { data, error } = await sb.storage.from("db-backups").createSignedUrl(filename, 120, { download:true });
    if(error||!data||!data.signedUrl){ alert("Could not prepare the download: "+((error&&error.message)||"unknown")); return; }
...
function renderMeetings(){
  ...
  ${meetingReimbPanel(rows,viewDate)}
  ...
  on("mtgTallyCsv","click",()=>meetingReimbCsv(rows,viewDate));
...
function mPayout(row){ const b=String(row.bank_name||"").toLowerCase(); if(b.includes("gcash")) return "GCash"; if(b.includes("union")) return "UB"; return null; }
function mAmount(row){ return Number(row.reimb_transport||0)+Number(row.reimb_lbc||0); }
function mAcct(row){ return String(row.bank_account_no||"").replace(/\s/g,""); }
function mMonToken(viewDate){ const dt=viewDate?new Date(viewDate+"T00:00:00"):new Date(); return dt.toLocaleDateString("en-US",{month:"short"})+String(dt.getFullYear()).slice(2); } // e.g. Jun26
function mReimbSplit(rows){
  const pay=[], unpay=[];
  (rows||[]).forEach(r=>{ const amt=mAmount(r); if(amt<=0) return; // nothing to pay, not an error
    const ch=mPayout(r), no=mAcct(r);
    if(ch && no) pay.push(r); else unpay.push(r); });
  return {pay,unpay};
}
// Build one bank-upload workbook matching Marjorie's "Standard Upload File" template exactly.
async function mBankFile(rows, channel, viewDate){
  await tkLoadSheetJS();
  const mon=mMonToken(viewDate);
  const isG = channel==="GCash";
  const header=[
    ["Remarks*", isG?("DS Fare Reimb "+mon):"PAYROLL"],
    ["Channel*", isG?"Instapay":"UnionBank"],
    ["Source Account", isG?"002160722991":"002160727230"],
    [],
    ["Beneficiary Code","Beneficiary Name","Beneficiary Account Number","Beneficiary Address","Beneficiary Bank Code","Amount","OUR/SHA","Purpose","Other Remarks"]
  ];
  const body=(rows||[]).filter(r=>mPayout(r)===channel && mAcct(r) && mAmount(r)>0).map(r=>{
    const name=String(r.account_holder||r.name||"").trim();
    return isG
      ? ["", name, mAcct(r), "Pasig City", "0093", mAmount(r), "", "1002", ("Fare "+mon).slice(0,15)]
      : ["", "", mAcct(r), "", "", mAmount(r), "", 2, name];
  });
  const aoa=header.concat(body);
  const ws=window.XLSX.utils.aoa_to_sheet(aoa);
  // Force account numbers + bank code to TEXT so GCash leading-zero mobiles / "0093" survive.
  for(let i=0;i<body.length;i++){ const rr=6+i; // row index in sheet (1-based), data starts at row 6
    ["C","E"].forEach(col=>{ const cell=ws[col+rr]; if(cell && cell.v!=="" && cell.v!=null){ cell.t="s"; cell.v=String(cell.v); } });
  }
  const wb=window.XLSX.utils.book_new();
  window.XLSX.utils.book_append_sheet(wb, ws, "Standard Upload File");
  const fname = isG ? ("InstaPay GCash "+mon+".xlsx") : ("UB Payroll DSFareReimb "+mon+".xlsx");
  window.XLSX.writeFile(wb, fname);
  return body.length;
}
async function meetingBankFiles(rows, viewDate, channel){
  const {pay}=mReimbSplit(rows);
  const n=await mBankFile(pay, channel, viewDate);
  if(!n) alert("No "+(channel==="GCash"?"GCash":"UnionBank")+" payouts to export for this meeting.");
}
// Human-readable tally CSV, sorted Agency/Direct → channel, with the breakdown accounting sorts by.
function meetingReimbCsv(rows, viewDate){
  const {pay,unpay}=mReimbSplit(rows);
  const cols=["Agency/Direct","Channel","Name","Account holder","Emp No","Store","GCash/Account no","Transport","LBC","Total"];
  const line=a=>a.map(v=>`"${String(v==null?"":v).replace(/"/g,'""')}"`).join(",");
  const key=r=>(mIsAgency(r)?"1":"0")+"|"+(mPayout(r)||"z")+"|"+mNorm(r.name);
  const sorted=pay.slice().sort((a,b)=>key(a).localeCompare(key(b)));
  const out=[line(cols)];
  sorted.forEach(r=>out.push(line([mAgencyOf(r),mPayout(r),r.name,r.account_holder||"",r.emp_no||"",r.store||"",mAcct(r),Number(r.reimb_transport||0),Number(r.reimb_lbc||0),mAmount(r)])));
  if(unpay.length){ out.push(line([])); out.push(line(["NOT PAYABLE — no GCash/UnionBank details (amount owed but no valid payout):"]));
    unpay.forEach(r=>out.push(line([mAgencyOf(r),(r.bank_name||"—"),r.name,r.account_holder||"",r.emp_no||"",r.store||"",mAcct(r)||"—","","",mAmount(r)]))); }
  const a=document.createElement("a"); a.href=URL.createObjectURL(new Blob([out.join("\n")],{type:"text/csv"}));
  a.download="reimbursement-tally-"+(viewDate||"meeting")+".csv"; a.click();
}
function meetingReimbPanel(rows, viewDate){
  const {pay,unpay}=mReimbSplit(rows);
  const bucket=(ag,ch)=>pay.filter(r=>(mIsAgency(r)===ag)&&mPayout(r)===ch);
  const cell=(ag,ch)=>{ const b=bucket(ag,ch); const t=b.reduce((s,r)=>s+mAmount(r),0); return `<div class="kpi"><div class="k-l">${ag?"Agency":"Direct"} · ${ch}</div><div class="k-n" style="font-size:16px;">${mPeso(t)}</div><div class="esub">${b.length} payee${b.length===1?"":"s"}</div></div>`; };
  const grand=pay.reduce((s,r)=>s+mAmount(r),0);
  const gc=pay.filter(r=>mPayout(r)==="GCash").length, ub=pay.filter(r=>mPayout(r)==="UB").length;
  const key=r=>(mIsAgency(r)?"1":"0")+"|"+(mPayout(r)||"z")+"|"+mNorm(r.name);
  const sorted=pay.slice().sort((a,b)=>key(a).localeCompare(key(b)));
  return `
  <div class="panel">
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;">
      <h2 style="margin:0;">Reimbursement payout ${viewDate?"· "+fmtDate(viewDate):""}</h2>
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <button class="btn" style="${SBTN}" id="mtgGcashFile"${gc?"":" disabled"}>Download GCash file (${gc})</button>
        <button class="btn" style="${SBTN}" id="mtgUbFile"${ub?"":" disabled"}>Download UB file (${ub})</button>
        <button class="btn ghost" style="${SBTN}" id="mtgTallyCsv">Download tally (CSV)</button>
      </div>
    </div>
    <div class="psub">Two bank-ready files, exactly in Marjorie's upload format: <b>GCash</b> → InstaPay (Pasig · 0093 · purpose 1002); <b>UnionBank</b> → payroll (purpose 2, name in remarks). Sorted below by Agency/Direct then channel so Accounting can book each group.</div>
    <div class="grid kpis" style="grid-template-columns:repeat(4,1fr);margin-top:12px;">
      ${cell(false,"GCash")}${cell(false,"UB")}${cell(true,"GCash")}${cell(true,"UB")}
    </div>
    <div style="margin-top:6px;font-weight:700;">Grand total to pay: ${mPeso(grand)} · ${pay.length} payee${pay.length===1?"":"s"}${unpay.length?` · <span style="color:#a4322a;">⚠ ${unpay.length} can't be paid (no GCash/UB)</span>`:""}</div>
    ${pay.length?`
    <div style="overflow-x:auto;margin-top:12px;">
    <table><thead><tr>
      <th>Agency/Direct</th><th>Channel</th><th>Name</th><th>Emp&nbsp;No</th><th>Store</th><th>GCash/Account&nbsp;no</th><th>Transport</th><th>LBC</th><th>Total</th>
    </tr></thead><tbody>
    ${sorted.map(r=>`<tr>
      <td>${mIsAgency(r)?`<span class="pill probation">${esc(mAgencyOf(r))}</span>`:'<span class="pill active">Direct</span>'}</td>
      <td><b>${mPayout(r)==="UB"?"UnionBank":"GCash"}</b></td>
      <td><b>${esc(r.name)}</b>${r.account_holder&&mNorm(r.account_holder)!==mNorm(r.name)?`<div class="esub">a/c: ${esc(r.account_holder)}</div>`:""}</td>
      <td>${esc(r.emp_no||"—")}</td>
      <td>${esc(r.store||"—")}</td>
      <td>${esc(mAcct(r))}</td>
      <td>${mPeso(r.reimb_transport)}</td>
      <td>${mPeso(r.reimb_lbc)}</td>
      <td><b>${mPeso(mAmount(r))}</b></td>
    </tr>`).join("")}
    </tbody></table></div>`:'<div class="psub" style="margin-top:10px;">No payable reimbursements yet.</div>'}
    ${unpay.length?`<div style="margin-top:12px;border:1px solid #f0d0cb;background:#fdf5f4;border-radius:10px;padding:11px 13px;">
      <div style="font-weight:700;color:#a4322a;">⚠ ${unpay.length} owed but not payable — no GCash / UnionBank on file</div>
      <div class="esub" style="margin-top:4px;">${unpay.map(r=>esc(r.name)+" ("+mPeso(mAmount(r))+(r.bank_name?" · "+esc(r.bank_name):"")+")").join(" · ")}</div>
      <div class="esub" style="margin-top:4px;">Ask them to re-submit with a GCash number or UnionBank account.</div>
    </div>`:""}
  </div>`;
}
...
function fpSplitField(key,label,hint,fp){
  ...
  const box=(sub,idv,val,ph)=>`<div style="flex:1;min-width:0;"><input id="fp_${key}_${sub}" data-fpsplit="${key}" type="text" inputmode="decimal" value="${val??""}" placeholder="${ph}" style="width:100%;padding:8px 10px;border:1px solid #e2e7e4;border-radius:7px;font-size:13.5px;"><div style="font-size:10px;color:#8a948e;text-transform:uppercase;letter-spacing:.3px;margin-top:2px;text-align:center;">${idv}</div></div>`;
...
function fpField(key,label,hint,fp){ const v=fp&&fp[key], n=fp&&fp[key+"_note"];
  return `<div style="margin-bottom:9px;"><label style="display:block;font-size:11px;font-weight:700;color:#6a766f;text-transform:uppercase;letter-spacing:.3px;margin-bottom:3px;">${label}</label>
    <input id="fp_${key}" type="text" inputmode="decimal" value="${v??""}" placeholder="0.00" style="width:100%;padding:8px 10px;border:1px solid #e2e7e4;border-radius:7px;font-size:13.5px;">
...
function collectFinalPay(){ const m=document.getElementById("exModal"); if(!m) return {};
  const num=id=>{ const e=m.querySelector("#fp_"+id); if(!e||e.value.trim()==="") return 0; const v=Number(String(e.value).replace(/[^0-9.\-]/g,"")); return isNaN(v)?0:v; };
...
function openExitCase(id){
  ...
  // Auto-remarks: every line gets an explanation. HMO is DATA-DRIVEN from the linked employee's enrollment
  // (0 with "no HMO on record" when they were never enrolled). Only fills a note that's still blank — never overwrites HR's.
  (function(){
    const _emp=linkedEmployeeForCase(x);
    const _hmoOn=!!(_emp && (_emp.hmo_enrolled || _emp.hmo_provider || _emp.hmo_member_no));
    const hmoNote=_hmoOn ? ("HMO "+[_emp.hmo_provider,_emp.hmo_member_no].filter(Boolean).join(" · ")+" — compute pro-rated reversal for unused coverage.") : "No HMO enrollment on record — nothing to reverse.";
    const DEF={
      last_payroll:"Unpaid salary for the last worked period.",
      thirteenth_month:"Pro-rated 13th-month pay for the year up to the last day.",
      leave_incentive:"Unused Service Incentive Leave converted to cash.",
      hmo_reversal:hmoNote,
      tax_refund:"Tax over-withheld for the year, refunded on final pay.",
      rebates_incentives:"Rebates due on incentives, if any.",
      commission:"Earned commission / incentives pending release.",
      breach_of_contract:"Liquidated damages per a signed contract clause — verify with counsel.",
      advances_office_loan:"Outstanding cash advance / office loan balance.",
      other_charges:"Other charges — itemize in this note.",
      inventory_short:"Inventory shortage per signed NTE / ATD — cite the reference #."
    };
    FP_ALL.forEach(([k])=>{ if(fp[k+"_note"]==null || String(fp[k+"_note"]).trim()===""){ fp[k+"_note"]=DEF[k]||""; } });
  })();
    ...
    <div style="padding:18px 22px 60px;">
      <div class="panel" style="margin-top:0;"><h2>Separation</h2>
        ...
        <div style="margin-top:10px;border-top:1px solid var(--line);padding-top:8px;">
          <label style="display:block;font-size:11px;font-weight:700;color:#6a766f;text-transform:uppercase;margin-bottom:4px;">Resignation letter / separation document</label>
          <div id="exResignList">${(Array.isArray(x.resignation_letter)?x.resignation_letter:[]).map(a=>`<div style="display:flex;align-items:center;gap:8px;padding:3px 0;font-size:12.5px;"><span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">📎 ${esc(a.name)}</span><button type="button" class="btn ghost" style="padding:2px 9px;font-size:11.5px;flex-shrink:0;" onclick="openLoanDoc('${esc(a.path)}',this)">View</button></div>`).join("")}</div>
          <input type="file" id="exResignFile" style="font-size:12.5px;margin-top:4px;">
          <div id="exResignMsg" style="font-size:11.5px;color:var(--muted);margin-top:3px;"></div>
        </div>
      </div>
      ...
      <div class="panel"><h2>Final pay — quitclaim computation ${x.quitclaim_status?qcPill(x.quitclaim_status):""}</h2>
        ...
        ${x.quitclaim_status==="Approved"?`<div class="note" style="background:var(--green-light);border-color:var(--green);">✓ Approved &amp; e-signed by the Director${x.quitclaim_date?" on "+fmtDate(x.quitclaim_date):""} — authorised for Accounting to process. HR: print, then record the release (check no. + proof) in Closeout below.</div>`:""}
      ...
      <div class="panel"><h2>Closeout</h2>
        ...
        ${(()=>{
          const approved = x.quitclaim_status==="Approved";
          const canRel = canReleaseFinalPay();
          const proof = Array.isArray(x.release_proof)?x.release_proof:[];
          if(x.released){
          ...
          return `<div style="border:1px solid ${approved?'#f0c14b':'var(--line)'};border-radius:9px;padding:11px 13px;margin:8px 0;background:${approved?'#fffaf0':'#f7faf8'};">
            <div style="font-weight:700;font-size:13.5px;">Final pay release ${approved?'<span style="color:#9a6a00;">— awaiting payout (with Accounting)</span>':'<span style="color:var(--muted);">— not authorised yet</span>'}</div>
            <div class="td" style="margin:3px 0 8px;">${approved
              ? `✓ Amount approved &amp; signed by the Director${x.quitclaim_date?" on "+fmtDate(x.quitclaim_date):""} — <b>authorised for Accounting to process</b>. Print the signed quitclaim (button above), hand it to Accounting, then record the <b>check / reference no.</b> and upload the <b>proof of claim</b> here to close the cycle.`
              : `⏳ The Director must sign the quitclaim above first — <b>that signature is what authorises Accounting to process</b> the payment. The release can't be recorded until then.`}</div>
            ${approved && canRel ? `
              <label style="display:block;font-size:11px;font-weight:700;color:#6a766f;text-transform:uppercase;letter-spacing:.3px;margin-bottom:3px;">Check / reference no.</label>
              <input id="ex_release_check_no" value="${esc(x.release_check_no||"")}" placeholder="e.g. Cheque 0012345 / bank transfer ref" style="width:100%;padding:8px 10px;border:1px solid #e2e7e4;border-radius:7px;font-size:13.5px;margin-bottom:9px;">
              <label style="display:block;font-size:11px;font-weight:700;color:#6a766f;text-transform:uppercase;letter-spacing:.3px;margin-bottom:3px;">Proof of claim / payment <span style="color:var(--muted);font-weight:400;text-transform:none;">— voucher, cheque copy, or transfer screenshot</span></label>
              <input type="file" id="exReleaseFile" multiple style="font-size:12.5px;margin-bottom:5px;">
              <div id="exReleaseFileList"></div>
              <div id="exReleaseMsg" style="font-size:12px;margin-top:4px;"></div>`
              : (approved && !canRel ? `<div class="psub" style="margin:0;">This is with <b>Accounting to process</b>. Only <b>HR (Juvy)</b> records the release (check no. + proof).</div>` : "")}
          </div>`;
        })()}
      </div>
      ...
      <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin-top:6px;">
        ...
        ${(()=>{ if(x.overall_status==="Complete") return ""; if(!canReleaseFinalPay()) return "";
          return '<button class="btn" id="exReleaseComplete" style="background:#1F6B52;color:#fff;">✓ Record release → close cycle</button>'; })()}
  ...
  (function(){ const rf=document.getElementById("exResignFile"); if(!rf) return; rf.onchange=async()=>{ const f=rf.files&&rf.files[0]; if(!f) return; const msg=document.getElementById("exResignMsg"); if(msg) msg.textContent="Uploading "+f.name+"…";
    const path="exits/"+x.id+"/resign-"+Date.now()+"-"+f.name.replace(/[^\w.\-]+/g,"_");
    const {error}=await sb.storage.from("loan-docs").upload(path,f,{upsert:true});
    if(error){ if(msg) msg.textContent="Upload failed: "+error.message; return; }
    const arr=Array.isArray(x.resignation_letter)?x.resignation_letter.slice():[]; arr.push({name:f.name,path,uploaded_at:new Date().toISOString(),by:(CURRENT_USER&&CURRENT_USER.email)||null});
    const e2=(await sb.from("exit_clearance").update({resignation_letter:arr,updated_at:new Date().toISOString()}).eq("id",x.id)).error;
    if(e2){ if(msg) msg.textContent="Saved file but couldn't record: "+e2.message; return; }
    await loadEmployees(); openExitCase(x.id); }; })();
...
// Proof-of-claim upload for the final-pay release — reuses the shared loan-docs bucket + jsonb list pattern.
function wireExitReleaseUpload(x){
  const listEl=document.getElementById("exReleaseFileList");
  const fInput=document.getElementById("exReleaseFile");
  if(!listEl && !fInput) return;
  const caseKey = x.id || ("case-"+Date.now());
  const render=()=>{ if(!listEl) return;
    listEl.innerHTML=_exReleaseProof.length ? _exReleaseProof.map((a,i)=>`<div style="display:flex;align-items:center;gap:8px;padding:4px 0;font-size:12.5px;"><span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">📎 ${esc(a.name)}</span>${a.path?`<button type="button" class="btn ghost" style="padding:2px 8px;font-size:11.5px;flex-shrink:0;" onclick="openLoanDoc('${esc(a.path)}',this)">View</button>`:""}<button type="button" class="btn ghost" data-rxr="${i}" style="padding:2px 8px;font-size:11.5px;flex-shrink:0;">Remove</button></div>`).join("") : `<div class="psub" style="margin:2px 0;">No proof attached yet.</div>`;
    listEl.querySelectorAll("[data-rxr]").forEach(b=>b.onclick=()=>{ _exReleaseProof.splice(+b.dataset.rxr,1); render(); });
  };
  render();
  if(fInput) fInput.onchange=async()=>{
    const files=[...fInput.files]; if(!files.length) return;
    const msg=document.getElementById("exReleaseMsg"); const setMsg=(t,c)=>{ if(msg){ msg.style.color=c||"#4a5560"; msg.textContent=t; } };
    for(const f of files){
      if(f.size>25*1024*1024){ setMsg(`“${f.name}” is over 25 MB — attach a smaller file.`, "#a12"); continue; }
      setMsg("Uploading "+f.name+"…");
      const path=`exit/${caseKey}/${Date.now()}-${f.name.replace(/[^\w.\-]+/g,"_")}`;
      const {error:upErr}=await sb.storage.from("loan-docs").upload(path,f,{upsert:true});
      if(upErr){ setMsg("Upload failed: "+upErr.message, "#a12"); continue; }
      _exReleaseProof.push({name:f.name, path, uploaded_at:new Date().toISOString(), uploaded_by:(CURRENT_USER&&CURRENT_USER.email)||null});
    }
    setMsg(""); fInput.value=""; render();
  };
}
...
async function saveExitCase(x,modal,mode){
  ...
  if(release){
    ...
    if(netDue>0){
      // Gate 1 — the amount must be Director-approved first: her signature is what authorises Accounting to process.
      if(x.quitclaim_status!=="Approved"){ document.getElementById("exMsg").textContent="The quitclaim isn't signed by the Director yet — that signature authorises Accounting to process. Send it for sign-off first."; return; }
...
function printQuitclaim(x){
  ...
  const claimRow=([k,l])=>{ const note=esc(fp[k+"_note"]||"");
    if(FP_SPLIT.has(k)){ const b=Number(fp[k+"_basic"]||0), a=Number(fp[k+"_allowance"]||0); const has=(b||a);
      return `<tr><td>${esc(l)}</td><td class="n sub">${has?P(b):""}</td><td class="n sub">${has?P(a):""}</td><td class="n">${P(fp[k])}</td><td class="rm">${note}</td></tr>`; }
    return `<tr><td>${esc(l)}</td><td class="n"></td><td class="n"></td><td class="n">${P(fp[k])}</td><td class="rm">${note}</td></tr>`; };
  const rowsC=FP_CLAIMS.filter(([k])=>Number(fp[k]||0)!==0||fp[k+"_note"]).map(claimRow).join("");
  const rowsD=FP_DEDUCTIONS.filter(([k])=>Number(fp[k]||0)!==0||fp[k+"_note"]).map(([k,l])=>`<tr><td>${esc(l)}</td><td class="n"></td><td class="n"></td><td class="n">${P(fp[k])}</td><td class="rm">${esc(fp[k+"_note"]||"")}</td></tr>`).join("");
  w.document.write(`<!DOCTYPE html><html><head><title>Quitclaim — ${esc(x.employee_name)}</title><style>
    ...
    .hd td{font-weight:700;background:#eef2f7;color:#1E3A5F;font-size:10.5px;text-transform:uppercase;letter-spacing:.3px;} td.sub{color:#667;font-size:11.5px;}
    ...
    <table><tr class="hd"><td>Item</td><td class="n">Basic</td><td class="n">Allowance</td><td class="n">Amount</td><td>Remarks</td></tr>
      ${rowsC||'<tr><td colspan="5">—</td></tr>'}<tr class="tot"><td>Total claims</td><td></td><td></td><td class="n">${P(fpClaims(fp))}</td><td></td></tr></table>
    <h3>Less: deductions</h3>
    <table><tr class="hd"><td>Item</td><td class="n">Basic</td><td class="n">Allowance</td><td class="n">Amount</td><td>Remarks</td></tr>
      ${rowsD||'<tr><td colspan="5">—</td></tr>'}<tr class="tot"><td>Total deductions</td><td></td><td></td><td class="n">${P(fpDeductions(fp))}</td><td></td></tr></table>
